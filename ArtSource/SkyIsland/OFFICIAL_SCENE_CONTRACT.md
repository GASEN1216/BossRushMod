...
| 撤离点标识 | 官方出口预制体自带可见的撤离区域 | `SkyIslandExtractionRings` 在 `Exit` / `BellExtraction` 脚下画贴地圆环（共享 `SkyIslandGroundRing`，与噬风预警圈同一建造点）：码头青色环（`BossRushUIColors.Accent`）恒亮、钟庭绿环随敲钟结局出现，半径即 `ExtractionRadius`。**作者场景不提供任何撤离视觉**——`tools/generate_sky_island.py` 的 `marker()` 建的是 Blender Empty，官方小地图也不标撤离点，所以这一层必须由代码补（CR-2026-09-09-012）|
