...
- **"???" boss drop**: killing "???" rolls once for the set: Frost Crown 20%, Ice Armor 20%, 60% nothing. It also has its separate 50% Frostmourne drop; the two never interfere. **Vanilla raids count too**, no BossRush run needed. Inside the arena it lands in the mod reward crate; in Infinite Hell it drops on the ground.
...
Activates when both pieces are equipped (screen notification appears). Your eyes glow pale blue and a ring of frost mist follows your feet.

**Passive:**
- Ice damage resistance **+50%**
...
- Uses the set's own freeze effect first; falls back to vanilla Cold, then to a temporary slow, if the asset is missing
...
- Base armor (Head +5 / Body +5) is low, so it's not a pure tank set; it lives on control

[tip] Counter-freeze is useful against enemies that chase into melee range. When it triggers, use the control window to create distance or attack.
