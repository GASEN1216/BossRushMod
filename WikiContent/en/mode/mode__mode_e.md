...
- Only hostile-faction deaths can pay Shells
- A direct player kill earns the full reward; otherwise, being within 8 meters when the Boss dies earns half
- Your first positive reward of the run includes 10 bonus Shells, so the economy can get moving
- A standard Boss is valued from its maximum health at spawn: 500 HP is roughly 10 Shells, and each health doubling adds about 3, with a minimum reward of 1
- Promoted minions pay 70% of the standard Boss value
...
- It only draws from the **currently open category**
- The price is the median Shell price of all items in that category
- Quality weights gradually improve during the first 30 minutes of the run
- The reward uses the normal delivery path; the button is disabled while funds are insufficient or another transaction is active
...
- Base price scales from maximum health: roughly 200 Shells at 1,000 HP, clamped from 50 to 2,000 and rounded up to the nearest 10
- Every currently living hire doubles the price of all later offers
- There is no gameplay cap on hires; your Shell income is the practical limit
- A hired Boss joins the player faction, follows you, and keeps its weapons and abilities
