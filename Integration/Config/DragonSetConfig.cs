...
namespace BossRush
{
    ...
    public static class DragonSetConfig
    {
        ...
        // 龙头属性
        private const float DRAGON_HELM_PHYSICS_RESIST = -0.1f;   // 物理承伤倍率固定-0.1
        ...
        private const float DRAGON_HELM_FIRE_FACTOR = -0.15f;     // 火承伤倍率固定-0.15
        private const float DRAGON_HELM_ELECTRIC_FACTOR = -0.15f; // 电承伤倍率固定-0.15
        private const float DRAGON_HELM_POISON_FACTOR = 0.3f;     // 毒承伤倍率固定+0.3
        ...
        // 龙甲属性
        private const float DRAGON_ARMOR_PHYSICS_RESIST = -0.2f;   // 物理承伤倍率固定-0.2
        private const float DRAGON_ARMOR_STORM_PROTECTION = 1f;    // 风暴防护+1
        private const float DRAGON_ARMOR_COLD_PROTECTION = 1f;     // 寒冷防护+1
        private const float DRAGON_ARMOR_FIRE_FACTOR = -0.2f;      // 火承伤倍率固定-0.2
        ...
        private static void ConfigureDragonHelm(Item item)
        {
            ...
            // 固定降低物理承伤倍率 0.1，避免被其他固定抗性再稀释
            EquipmentHelper.AddModifierToItem(item, "ElementFactor_Physics", ModifierType.Add, DRAGON_HELM_PHYSICS_RESIST, true);

            // 固定降低火承伤倍率 0.15
            EquipmentHelper.AddModifierToItem(item, "ElementFactor_Fire", ModifierType.Add, DRAGON_HELM_FIRE_FACTOR, true);

            // 固定降低电承伤倍率 0.15
            EquipmentHelper.AddModifierToItem(item, "ElementFactor_Electricity", ModifierType.Add, DRAGON_HELM_ELECTRIC_FACTOR, true);

            // 固定提高毒承伤倍率 0.3
            EquipmentHelper.AddModifierToItem(item, "ElementFactor_Poison", ModifierType.Add, DRAGON_HELM_POISON_FACTOR, true);
        ...
        private static void ConfigureDragonArmor(Item item)
        {
            ...
            // 固定降低物理承伤倍率 0.2，避免被其他固定抗性再稀释
            EquipmentHelper.AddModifierToItem(item, "ElementFactor_Physics", ModifierType.Add, DRAGON_ARMOR_PHYSICS_RESIST, true);

            // 固定降低火承伤倍率 0.2
            EquipmentHelper.AddModifierToItem(item, "ElementFactor_Fire", ModifierType.Add, DRAGON_ARMOR_FIRE_FACTOR, true);

            // 固定降低电承伤倍率 0.2
            EquipmentHelper.AddModifierToItem(item, "ElementFactor_Electricity", ModifierType.Add, DRAGON_ARMOR_ELECTRIC_FACTOR, true);

            // 固定提高毒承伤倍率 0.4
            EquipmentHelper.AddModifierToItem(item, "ElementFactor_Poison", ModifierType.Add, DRAGON_ARMOR_POISON_FACTOR, true);
