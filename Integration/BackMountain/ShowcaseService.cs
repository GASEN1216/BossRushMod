// ============================================================================
// ShowcaseService.cs - 陈列加成：官方枪械展示架 / 假人上实际摆放的 Mod 战利品 → 全局最大生命加成
// ============================================================================
// 【2026-09-22 改造：自建「战利品登记簿」退役，改接官方展示建筑】
//   官方基地能建枪械展示架 / 假人（Duckov.Buildings.Showcase；「陈列柜」是废弃建筑，建不了）：物品真被搬进去、
//   官方自己持久化、用物品自己的模型陈列——这正是自建柜给不了的（柜内三件展品是 GLB 烘死的）。
//   Mod 只做两件事：判定哪些 Mod 物品算战利品（ShowcaseTrophyCatalog，不补标签），以及把「官方架子上
//   现在摆着的 Mod 战利品」采集成快照（ShowcaseDisplayScanner）→ 本文件缓存、按品质挂加成。
//
// 【玩法取舍与回退】旧版是「登记不收走」（怕玩家在留装备与几个百分点之间二选一）。
//   官方架子是基地存储、不带出击、死了不掉，摆上去损失的是「使用」不是「拥有」，换来真实陈列；
...
namespace BossRush
{
    ...
    internal static class ShowcaseService
    {
        ...
        /// <summary>
        /// 把「官方枪架 / 假人上现在摆着的 Mod 战利品」整体覆盖为当前陈列。
        /// 与缓存一致时不落盘、不重挂加成（避免每次 plug 都写存档）；老登记簿只在基地找到官方展示建筑时才被覆盖。
