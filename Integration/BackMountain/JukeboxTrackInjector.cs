...
//   按音频路径判重：切语言只改原条目的标题，索引和当前选曲保持稳定。
//
...
namespace BossRush
{
    ...
    internal static class JukeboxTrackInjector
    {
        ...
        internal static void EnsureInjected()
        {
            try
            {
                ...
                for (int i = 0; i < tracks.Count; i++)
                {
                    ...
                    int index = FindTrack(selector.entries, path);
                    if (index >= 0) selector.entries[index] = entry;
                    else { selector.entries.Add(entry); added++; }
                }
        ...
        private static int FindTrack(List<BaseBGMSelector.Entry> entries, string path)
        {
            for (int i = 0; i < entries.Count; i++)
            {
                if (string.Equals(entries[i].filePath, path, StringComparison.OrdinalIgnoreCase)) return i;
            }
            return -1;
        }
    }
