...
//   重复模板抽到一个公共入口，给 Mode D / Mode E / Mode F / 标准 BossRush
//   复用，避免每个模式自己写一遍 try-catch + 数值 Clamp + UI 触发。
//
// 设计原则：
...
namespace BossRush
{
    ...
    {
        /// <summary>
        /// 给当前模式抽并应用变异词条（标准 BossRush 已直接调 MutatorManager.RollAndApply，
        /// 这里给 Mode D / E / F 等"非标准 BossRush 流程"用）
        /// </summary>
        /// <param name="modeTag">日志前缀，例如 "ModeD" / "ModeE" / "ModeF"</param>
