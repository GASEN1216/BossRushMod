// ============================================================================
// CodexSaveCoordinator.cs - 鸭皇图鉴落盘协调器
// ============================================================================
// 硬约束（形态照 Integration/DailyReport/DailyReportSaveCoordinator.cs）：
//   - 本类是图鉴**唯一**调用 SavesSystem.SaveFile 的地方，且每批至多一次；
//     采集器只调 CodexPersistence.Store()（入队），绝不自己落盘。
//   - SavesSystem.IsSaving 时不强写，只登记 deferred，由宿主 tick 重试；
//   - 物理落盘只发生在基地场景：SaveFile 会做备份拷贝 + 整档同步写盘，而击杀天然
//     发生在交火帧上；非基地一律推迟到回基地补写，宿主销毁与关停走 bypassSceneGate
//     兜底（最后机会，宁可写一次也不丢收藏进度）。
//     非基地的 Tick 不消耗 deferred 重试预算，否则长时间战斗会把 pending
//     丢成 budget_exhausted；
...
namespace BossRush
{
    /// <summary>图鉴落盘协调器。pending 合并成一批，一批一次 SaveFile。</summary>
    internal static class CodexSaveCoordinator
    {
        ...
        internal static bool TryFlushOnHostDestroy()
        {
            ...
            {
                string error;
                // 销毁/关停是最后机会，必须绕过场景闸：宁可在战斗帧写一次，也不能丢收藏进度
        ...
        private static bool FlushBatch(out string error, bool bypassSceneGate)
        {
            ...
            try
            {
                // 官方 SaveFile 会做备份文件拷贝 + 整档同步写盘。图鉴的写入点就是击杀 Boss
                // 那一帧，必然落在交火中，因此非基地一律推迟；pending 仍在持久层，
                // 回基地由 Tick 补写，官方任何一次存盘也会顺带带走。
            ...
            catch (Exception e)
            {
                ...
                _lastError = error;
                lock (_lock) { _deferredFlushPending = true; }
                ModBehaviour.DevLog(CodexTuning.LogPrefix + "[ERROR] 存档批次落盘异常: " + e.Message);
                return false;
            }
        }
