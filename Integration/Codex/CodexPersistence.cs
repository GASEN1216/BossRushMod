// ============================================================================
// CodexPersistence.cs - 鸭皇图鉴存档管线
// ============================================================================
// 硬约束（形态照 Integration/DailyReport/DailyReportPersistence.cs，语义按单 key 收敛）：
    ...
    internal static class CodexPersistence
    {
        ...
        internal static void EnsureSubscribed()
        {
            ...
            lock (_subscriptionLock)
            {
                ...
                {
                    ModBehaviour.DevLog(CodexTuning.LogPrefix + "[WARNING] 存档订阅失败: " + e.Message);
                }
            }
        ...
        /// <summary>
        /// 官方存盘前的采集点。图鉴没有「运行时余数」要同步（击杀在发生的那一刻
        /// 就已经进了 DTO 并入队），因此这里只把 pending 合并进 ES3 缓存，
        /// **不单独** SaveFile（那是协调器的唯一职责）。
        /// </summary>
        ...
        internal static CodexData LoadOrInit()
        {
            ...
            CodexData loaded = LoadOrInitCore(ref slotDrifted);

            // 槽位漂移的复位通知放在锁外：运行时侧（协调器 / 采集器 / 目录）各有自己的锁，
            // 在持久层锁内回调会引入跨锁顺序。
            if (slotDrifted)
            {
                ModBehaviour.DevLog(CodexTuning.LogPrefix
                    + "[WARNING] 检测到存档槽位已变更但未收到切档回调，图鉴缓存已自失效并从新槽重载");
                NotifySlotDrift();
            }
            return loaded;
        }
        ...
        private static CodexData LoadOrInitCore(ref bool slotDrifted)
        {
            lock (_lock)
            {
                ...
                try
                {
                    ...
                    keyExists = SavesSystem.KeyExisits(CodexTuning.StorageKey);
                }
                catch (Exception e)
                {
                    _writeBarrier = true;
                    ...
                    _cache = CodexCodec.CreateDefault();
                    return _cache;
                }
                ...
                try
                {
                    raw = SavesSystem.Load<string>(CodexTuning.StorageKey);
                }
                catch (Exception e)
                {
                    _writeBarrier = true;
                ...
                int version = CodexCodec.ReadSchemaVersion(raw);
                if (version != CodexTuning.CurrentSchemaVersion)
                {
                    // 高版本 fail-closed 只读；低版本目前没有迁移路径，同样只读不覆盖。
                    _writeBarrier = true;
                    _lastError = "schema_mismatch:" + version;
                    ModBehaviour.DevLog(CodexTuning.LogPrefix + "[WARNING] 存档 schemaVersion="
                        + version + " 与当前 " + CodexTuning.CurrentSchemaVersion
                        + " 不符，只读不覆盖");
                    _cache = CodexCodec.CreateDefault();
                    return _cache;
                }

                CodexData decoded = CodexCodec.Decode(raw);
                if (decoded == null)
                {
                    _writeBarrier = true;
                    _lastError = "decode_failed";
                    ModBehaviour.DevLog(CodexTuning.LogPrefix + "[WARNING] 存档解码失败，进入写屏障");
                    _cache = CodexCodec.CreateDefault();
                    return _cache;
                }
        ...
        internal static CodexData Current { get { return LoadOrInit(); } }
        ...
        /// 保证「数据换了、采集与目录也换」——否则新槽会继承上一个槽的去重集与目录快照。
        /// </summary>
        ...
        internal static bool Store(CodexData value)
        {
            ...
            try
            {
                ...
                string json = CodexCodec.Encode(value);
        ...
        internal static bool FlushPending()
        {
            lock (_lock)
            {
                ...
                try
                {
                    ...
                    SavesSystem.Save<string>(CodexTuning.StorageKey, _pendingJson);

                    // 回读核对：写进去的字符串必须能原样读回来
                    string readback = SavesSystem.Load<string>(CodexTuning.StorageKey);
                    if (!string.Equals(readback, _pendingJson, StringComparison.Ordinal))
                    {
                        throw new InvalidOperationException("codex save readback mismatch");
                    }
