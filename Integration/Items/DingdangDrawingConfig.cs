    ...
    public static class DingdangDrawingConfig
    {
        ...
        public static void ConfigureItem(Item item)
        {
            ...
            try
            {
                ...
                // 2. 添加 UsageUtilities 组件
                // 注意：测试仅通过 OnUse 中不调用 Consume() 来防止消耗，不设置耐久度
        ...
        public static void InjectLocalization()
        {
            try
            {
                ...
                // 注入描述
                LocalizationHelper.InjectLocalization(LOC_KEY_DESC, isChinese ? DESCRIPTION_CN : DESCRIPTION_EN);
