...
namespace BossRush
{
    ...
    public static class AwenLootSweepTokenConfig
    {
        ...
        private static void SetItemTypeId(Item item, int typeId)
        {
            ...
            try
            {
                PropertyInfo prop = typeof(Item).GetProperty("TypeID", flags);
                if (prop != null && prop.CanWrite)
                {
                    prop.SetValue(item, typeId, null);
                }
            }
            catch { }
            ...
            try
            {
                FieldInfo field = typeof(Item).GetField("typeID", flags);
                if (field != null)
                {
                    field.SetValue(item, typeId);
                }
            }
            catch { }
            ...
            try
            {
                FieldInfo itemDataField = typeof(Item).GetField("itemData", flags);
                if (itemDataField != null)
                {
                    object itemData = itemDataField.GetValue(item);
                    if (itemData != null)
                    {
                        Type itemDataType = itemData.GetType();
                        PropertyInfo dataProp = itemDataType.GetProperty("typeID", flags);
                        if (dataProp != null && dataProp.CanWrite)
                        {
                            dataProp.SetValue(itemData, typeId, null);
                        }

                        FieldInfo dataField = itemDataType.GetField("typeID", flags);
                        if (dataField != null)
                        {
                            dataField.SetValue(itemData, typeId);
                        }

                        dataField = itemDataType.GetField("TypeID", flags);
                        if (dataField != null)
                        {
                            dataField.SetValue(itemData, typeId);
                        }
