...
2026-09-24 修订：主计划已按当日审核全面改写（范围改为上下文治理、宿主状态提取、复用与耦合点、两处目录归位；不再做全仓搬迁、Frameworks 分层、Roslyn / IL 分析器与逐文件台账，理由见主计划 §2.3）。本记录保留最初研究依据；文中对主计划章节号的引用以及“第一轮试点”“批次 0”的说法对应 09-22 旧版结构，不再作为交付范围。第 3 节 A1–A15 的设计修订仍是新版 §2、§4、§9 的依据。
