...
2026-09-24（COMPAT / WIRE+）：菜地收获横幅新增两处宿主引用，均属 Keep: Unity owner。Harmony 入口捕获当前宿主，交付完成时核对宿主仍为同一实例，阻止卸载或重载后的旧异步通知；复用现有 ShowBigBanner，不增加服务层或每帧扫描。
