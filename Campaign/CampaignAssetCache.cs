...
namespace BossRush
{
    ...
    internal static class CampaignAssetCache
    {
        ...
        /// <summary>官方 SodaCraft/SodaCharacter（角色、箱子、建筑）的颜色字段；它没有 _Color / _BaseColor / _TintColor。</summary>
        private static readonly int CampaignBossSodaTintProperty = Shader.PropertyToID("_Tint");
        ...
        internal static void SetRendererColor(Renderer renderer, Color color)
        {
            ...
            if (sharedMaterial != null && sharedMaterial.HasProperty(CampaignBossSodaTintProperty))
            {
                campaignFinalBossColorBlock.SetColor(CampaignBossSodaTintProperty, color);
            }
            else if (sharedMaterial != null && sharedMaterial.HasProperty(CampaignBossColorProperty))
            {
        ...
        /// <summary>
        /// 召唤石共用一份材质；颜色用属性块，卸载由本缓存统一回收。
        /// 首选官方角色 / 建筑着色器 SodaCraft/SodaCharacter（颜色字段 _Tint，自带 UniversalGBuffer，与原版画风一致）；
        /// 作者工程的 URP/Lit 在游戏里曾实测不显示，只留作找不到官方着色器时的兜底（VA-25）。
        /// </summary>
