...
echo(DebugAndTools\SkyIsland\SkyIslandRendering.cs
echo(DebugAndTools\SkyIsland\SkyIslandLighting.cs
echo(DebugAndTools\SkyIsland\SkyIslandSearchPoint.cs
...
echo(DebugAndTools\SkyIsland\SkyIslandLootTables.cs
echo(DebugAndTools\SkyIsland\SkyIslandLootPools.cs
echo(DebugAndTools\SkyIsland\SkyIslandMapFog.cs
...
echo(Integration\NewWeapons\FrostSpear\FrostSpearRuntime.cs
...
if %BUILD_EXIT_CODE% EQU 0 (
    ...
    if errorlevel 1 (
        ...
        if exist "Assets\arenas\sky_island_raid" (
            if not exist "%GAME_PATH%\Duckov_Data\Mods\%MOD_NAME%\Assets\arenas" mkdir "%GAME_PATH%\Duckov_Data\Mods\%MOD_NAME%\Assets\arenas"
            copy /Y "Assets\arenas\sky_island_raid" "%GAME_PATH%\Duckov_Data\Mods\%MOD_NAME%\Assets\arenas\sky_island_raid" >nul 2>nul
            echo Deployed Sky Island raid bundle to: %GAME_PATH%\Duckov_Data\Mods\%MOD_NAME%\Assets\arenas
        )
        if exist "Assets\Data\SkyIsland" (
            if not exist "%GAME_PATH%\Duckov_Data\Mods\%MOD_NAME%\Assets\Data\SkyIsland" mkdir "%GAME_PATH%\Duckov_Data\Mods\%MOD_NAME%\Assets\Data\SkyIsland"
            xcopy /Y /I "Assets\Data\SkyIsland\*.json" "%GAME_PATH%\Duckov_Data\Mods\%MOD_NAME%\Assets\Data\SkyIsland\" >nul
            echo Deployed Sky Island data JSON to: %GAME_PATH%\Duckov_Data\Mods\%MOD_NAME%\Assets\Data\SkyIsland
        )
        ...
        rem P0 new weapon bundles (500048-500052). Item prefabs live in Assets\Items, melee/totem models in Assets\Equipment.
        rem Without these the runtime falls back to NewWeaponPlaceholderRegistry: the weapons still work but wear cloned models.
        for %%B in (viperdagger_item summonstaff_item energyshield_item frostspear_item thunderring_item) do (
            if exist "Assets\Items\%%B" (
                if not exist "%GAME_PATH%\Duckov_Data\Mods\%MOD_NAME%\Assets\Items" mkdir "%GAME_PATH%\Duckov_Data\Mods\%MOD_NAME%\Assets\Items"
                copy /Y "Assets\Items\%%B" "%GAME_PATH%\Duckov_Data\Mods\%MOD_NAME%\Assets\Items\%%B" >nul 2>nul
                if errorlevel 1 echo WARNING: new weapon item bundle deploy failed: %%B
            )
        )
        for %%B in (viperdagger_melee_model summonstaff_melee_model frostspear_melee_model energyshield_totem_model) do (
            if exist "Assets\Equipment\%%B" (
                ...
                copy /Y "Assets\Equipment\%%B" "%GAME_PATH%\Duckov_Data\Mods\%MOD_NAME%\Assets\Equipment\%%B" >nul 2>nul
                if errorlevel 1 echo WARNING: new weapon model bundle deploy failed: %%B
            )
        )
