...
//
// 策略（owner 2026-08-31 定）：遗种巢 / 日报 / 图鉴 / 词缀锻造 /
// 鸭王征程 / 竞技场后山 / 百战留痕这些玩法系统属于**默认内容**，不是可配置项。
// 它们的总开关不再注册进 ModConfig UI，玩家看不到、也不需要拨。
// 随机事件由玩家手动开关；后山调试旁路只保留 Dev 内部读口，不注册设置。
//
// 为什么光撤 UI 注册还不够：这些开关在历史版本里曾注册进 ModConfig，
...
// 因此除了撤注册，还必须：
//   1. LoadConfigFromModConfig 不再读这些恒开键（见 Config/Config.cs）；
...
namespace BossRush
{
    public partial class ModBehaviour
    {
        ...
        private void ForceContentSystemSwitchesOn()
        {
            ...
            // backMountainUnlockAll 仅供 Dev 使用；正式构建忽略历史值。
        }
    }
}
