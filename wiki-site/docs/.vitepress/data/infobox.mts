...
export const INFOBOX: Record<string, Infobox> = {
  ...
  '/equipment/viper-dagger': {
    ...
    rows: [
      ...
      { zh: '毒性爆发', en: 'Toxic Burst', vz: '满 5 层：35 点 + 这 5 刀实伤的 20%', ve: 'At 5 stacks: 35 flat + 20% of those 5 hits\' damage' },
      { zh: '获取', en: 'Availability', vz: '典狱长掉落 20% / 叮当商店（好感 5 级）', ve: '20% from the Warden / Dingdang\'s Shop (Affinity 5)' },
  ...
  '/equipment/frost-spear': {
    ...
    rows: [
      ...
      { zh: '获取', en: 'Availability', vz: '大冰冰掉落 20% / 叮当商店（好感 5 级）', ve: '20% from Big Ice / Dingdang\'s Shop (Affinity 5)' },
  ...
  '/equipment/summon-staff': {
    ...
    rows: [
      ...
      { zh: '灵魂投射', en: 'Soul Projection', vz: '瞄准方向前方 6 米召 3 只，存活 12 秒，冷却 12 秒', ve: '3 warriors 6m ahead of your aim, 12s lifetime, 12s cooldown' },
      { zh: '灵魂爆裂', en: 'Soul Burst', vz: '消散或阵亡时 3 米内 45 点灵魂伤害', ve: '45 ghost damage in 3m when a warrior fades or falls' },
      { zh: '获取', en: 'Availability', vz: '大兴兴掉落 20% / 叮当商店（好感 5 级）', ve: '20% from Big Xing / Dingdang\'s Shop (Affinity 5)' },
  ...
  '/equipment/flight-totem': {
    ...
    rows: [
      ...
      { zh: '死亡掉落', en: 'Drops on death', vz: '装备在槽位时保留', ve: 'Kept while equipped in its slot' },
      { zh: '来源', en: 'Source', vz: '焚天龙皇（15% 掉率）', ve: 'Skyburner Dragon Lord (15%)' },
  ...
  '/equipment/energy-shield': {
    ...
    rows: [
      ...
      { zh: '获取', en: 'Availability', vz: '呆头鹅掉落 20% / 叮当商店（好感 5 级）', ve: '20% from Goofy Goose / Dingdang\'s Shop (Affinity 5)' },
  ...
  '/equipment/thunder-ring': {
    ...
    rows: [
      { zh: '品质', en: 'Rarity', vz: '5', tier: 5 },
      { zh: '蓄力', en: 'Charge', vz: '每次被攻击命中 +1 层，最多 5 层（持续伤害不计）', ve: '+1 stack per hit taken, max 5 (damage over time excluded)' },
      { zh: '蓄力冷却', en: 'Stack cooldown', vz: '0.3 秒', ve: '0.3 s' },
      { zh: '衰减', en: 'Decay', vz: '8 秒未受击清空', ve: 'All stacks drop after 8 s without a hit' },
      { zh: '释放', en: 'Release', vz: '40 点 + 这一击实伤的 60%', ve: '40 flat + 60% of that hit\'s damage' },
      { zh: '获取', en: 'Availability', vz: '三枪哥掉落 20% / 叮当商店（好感 5 级）', ve: '20% from Triple-Shot Man / Dingdang\'s Shop (Affinity 5)' },
  ...
  '/equipment/dragon-set': {
    ...
    rows: [
      ...
      { zh: '套装效果', en: 'Set bonus', vz: '火伤免疫，按火焰部分实际伤害的 80% 治疗', ve: 'Fire immunity; heal for 80% of the fire damage component' },
  ...
  '/equipment/sky-island-starworks-gear': {
    ...
    rows: [
      ...
      { zh: '岛上用处', en: 'On the isles', vz: '星铜盔 / 星炉甲 / 星炉背囊任穿两件：渡口工台少耗 1 片残铜片；观星镜盔站定标敌', ve: 'Any two helm / harness / pack pieces: one less brass scrap at the dock; the lens helm spots enemies' },
  ...
  '/game-modes/mode-h': {
    ...
    rows: [
      ...
      { zh: '装备要求', en: 'Loadout', vz: '不限，通常在看台指挥', ve: 'Anything — normally manage from the stands' },
      { zh: '结构', en: 'Structure', vz: '一季六场，签两名斗士', ve: 'Six bouts a season, two signed fighters' },
      { zh: '你能做的', en: 'Your input', vz: '选人、看盘、押钱、每场喊一次口令', ve: 'Pick fighters, read odds, bet money, one call per bout' },
      { zh: '变异词条', en: 'Mutators', vz: '不抽', ve: 'No' },
      { zh: '地图', en: 'Maps', vz: '能摆下擂台的 BossRush 地图', ve: 'BossRush maps that fit an arena' },
      { zh: '难度', en: 'Difficulty', vz: '★★★☆☆' },
    ],
  },
  ...
  '/npcs/courier': {
    ...
    rows: [
      ...
      { zh: '整包寄存道具', en: 'Bulk shipping item', vz: '阿稳的快递令', ve: "Awen's courier token" },
    ],
