...
- 现在掉落兜底必须委托 BossRushDynamicItemRegistry，避免多处重复维护 bundle 名
"""
...
def main() -> int:
    ...
    if "BossRushDynamicItemRegistry.EnsureRegistered(PhantomWitchScytheIds.WeaponTypeId)" not in helper_block:
        return fail("PhantomWitchScytheRewardBundleGuard: helper must call BossRushDynamicItemRegistry.EnsureRegistered")

    if 'EquipmentOnly("phantom_scythe")' not in registry:
        return fail("PhantomWitchScytheRewardBundleGuard: unified registry must map 500044 to phantom_scythe")
