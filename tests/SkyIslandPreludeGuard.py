...
def check(sources):
    ...
    for token, why in (
        ...
        ("PayReward = null", "奖金必须在故事事务内提交，不再在事实落盘后发放"),
        ("story.TryDeliverQuest(action, rewardMoney, out message)", "交付没有通过现金事务"),
        ("DefaultCommit(def.DeliveredFlag, def.DeliverAction, out message, def.RewardMoney)", "任务奖励金额未进入交付事务"),
...
def main():
    ...
    probes = (
        ...
        (QUEST, "PayReward = null", "PayReward = () => PayAgain()"),
        (QUEST, "story.TryDeliverQuest(action, rewardMoney, out message)", "story.TryApply(action, out message)"),
        (QUEST, "DefaultCommit(def.DeliveredFlag, def.DeliverAction, out message, def.RewardMoney)", "DefaultCommit(def.DeliveredFlag, def.DeliverAction, out message)"),
