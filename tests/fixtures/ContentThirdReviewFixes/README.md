# 日报与图鉴第三轮审查回归

运行：`python tools/run_runtime_regressions.py --filter ContentThirdReviewFixes`（需要 .NET 8 SDK）。

覆盖 CR-2026-09-06-009、010、015。项目通过相对路径直接编译生产源码，C# 7.3；没有复制产品算法。日报链接完整 Service、Models、Codec、Tuning、SimpleJsonHelper 与共享 BossRushJsonValue，存储替身每次接收候选均经过真实编码再解码。旧档用没有扩展字段的 v1 JSON 字面量验证。

- 日报：已知故障禁发物；预写故障、发后故障与恢复；断签、换期、同一期不同连续签到的同格位债务；原品质/seed/签到日/格位冻结；重载；Store 拒绝保持原状态；旧掩码已领不重复；零 seed 冻结；损坏欠奖拒收。
- 图鉴：真实全局击杀采集器接收战役冠军 key，目录立即展示；补齐最后缺失 key 才发全录；重复击杀不重建；等数量但缺实际 key 不发全录；面板重判与切槽历史清理。

2026-09-22（CR-2026-09-22-011）：以实际官方冰原掠夺者的 `isBossCharacter=false`、真实名单中的 `Cname_RaiderIce` 驱动生产采集器，验证首击计时、四字段记录、去重、Codec 重载和最后一格全收集。名单兜底仍须经过非玩家归属、Mode H、随从、友军、基地、玩家自身过滤；同名丧尸 marker 优先，官方非 Boss 生物不能因展示名单而被误收录。

边界：健康、成就系统、存储 I/O、游戏时钟及奖励配送使用可注入替身。这里验证产品状态机、编解码与调用顺序，不代表 Unity、ES3、背包或掉落实物已实机验证。官方 Windows 编译与全量守卫由主审统一执行。


2026-09-18 图鉴追加：链接真实 Codec、共享 JSON 节点解析器与 SimpleJsonHelper。存储替身可注入 Store 拒绝、写屏障和永久故障；接受时经过真实 Encode/Decode 核验，不模拟 ES3 物理落盘。覆盖首次/重复击杀拒写不改变已提交对象、故障不发奖、已保存速杀补判、整数饱和、公共池自定义分类、同目录对象语言切换、零击杀历史项、损坏/超限存档整份拒读、64 个在战 Boss 容量保护、转友军后的死亡清理；直接驱动真实采集器验证五类丧尸 marker 归属、同一实例重复死亡以及 Mode H、玩家、友军、基地、随从、杂兵排除。10,000 次丧尸 key 查询检查 .NET 分配计数；该数字不是 Unity 帧时间采样。

2026-09-18 日报扩展：继续复用同一个夹具，新增链接真实 Bounty、Content、StatsCollector 和 ModeHSeedStream。覆盖全部五类/十三种不同目标组合、零死亡必须实际撤离、跨日与连续欠款、旧档原档位金额、拒写不发款、中英版面状态、首开种子、时钟缺席/停表、敌方角色归属、环境承伤、事件订阅和热采集零分配。后者只测替身环境中的托管分配，不能推断真实 Unity 帧耗。

2026-09-19 日报追加：13 种目标组合全部从真实采集器入口推进，经跨日结算与真实 Codec 重载后核对一次性付款；测试剩余时间随倍率变化、创刊号/第二期、零死亡三种状态及统计饱和。`run.py` 另从生产 UI/报箱文件逐字抽取排版、缩放、悬赏正文与注册方法，记录 SHA-256；替身只提供文本所需高度、RectTransform 和官方建筑列表，验证双栏不重叠、滚动容器包住末行、保留滚动位置、签到长说明不压按钮/短说明恢复、四档容器适配、依赖缺席拒收、缺 prefab 补齐和销毁后的替换。这里不模拟 TMP 字形、真实 View 装配、像素和 ES3。

2026-09-19 人工实测追加：普通签到发品质 2 小礼；同一天重复签到/打开/重载不重复发，旧档当天补偿一次，失败后断签/跨日/重载保留原奖品身份，里程碑同时发两份且每日礼不污染里程碑掩码。UI 排版回归直接提取生产 PanelWidth/PanelHeight/Margin，避免夹具尺寸常量落后产品。
