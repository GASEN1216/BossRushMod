...
class Program
{
    ...
    {
        int calls;
        Reset(); ItemUtilities.ThrowAfterOwnership = true; calls = 0; Func<bool> r = () => { calls++; return true; }; Check(SkyIslandItems.TryGive(BossRushItemIds.SkyIslandHomecomingBadge, false, r), "ownership before notification throw counts as delivered"); Check(calls == 1, "successful record is written once");
        Reset(); ItemUtilities.ThrowAfterBuffer = true; calls = 0; r = () => { calls++; return true; }; Check(SkyIslandItems.TryGive(BossRushItemIds.SkyIslandHomecomingBadge, true, r), "precise buffer receipt survives post-buffer throw"); Check(PlayerStorage.IncomingItemBuffer.Count == 1, "buffer receipt remains available");
