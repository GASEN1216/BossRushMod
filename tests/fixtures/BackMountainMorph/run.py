"""Run the complete production backyard morph runtime with controlled host adapters."""
...
OUT = ROOT / "Build/backmountain-morph"
...
def main():
    ...
    (OUT / "source-hashes.txt").write_text("\n".join(
        hashlib.sha256(p.read_bytes()).hexdigest() + " " + str(p.relative_to(ROOT)) for p in paths), encoding="utf-8")
