...
//   账本（一条进行中的押注 + 分档统计）是本槽的 typed 存档；扣钱 / 发钱之后把官方 EconomyData 快照
...
//
// 押背包物品（kind = KindItems，物品侧在 ModeHItemBetStake）：锁盘时不动钱、不动物品，只记下押了哪几件、估值与品质；
...
namespace BossRush
{
    ...
    /// <summary>押物品时账本里的一件：typeId|数量|估值|品质|名字，件与件之间用分号。名字只给界面看，去掉分隔符。</summary>
    internal sealed class ModeHItemBetEntry
    {
        ...
        internal static List<ModeHItemBetEntry> Decode(string text)
        {
            ...
            {
                string[] fields = parts[i].Split(new[] { '|' }, 5);
                ...
                result.Add(new ModeHItemBetEntry { TypeId = typeId, Count = count, Value = value, Quality = quality, Name = fields[4] });
            }
            return result;
        }
    ...
    internal static class ModeHCashBetService
    {
        ...
        /// <summary>锁盘时押上背包物品：只记账（押了哪几件、估值多少），不动钱也不动物品。</summary>
        ...
        /// <summary>技术中止 / 放弃赛季 / 孤儿押注：原样退回（至多一次）。没有挂着的押注时返回 false。</summary>
        ...
        /// <summary>账本 + 现金快照同批落盘（照 AchievementRewardJournal）。</summary>
        private sealed class CashBetJournal : IBossRushSaveBatchSource
        {
            ...
            internal CashBetJournal()
            {
                ...
                {
                    StorageKey = StorageKey, SchemaVersion = 1, LogPrefix = "[ModeH] ", DisplayName = "鸭王杯押钱账本",
            ...
            internal bool TrySettle(string runId, int matchIndex, bool won, long lossCharge, long winCash, string prizes, out long payout)
            {
                ...
                {
                    // 押物品：押上的东西没离开背包。赢了发奖品（由调用方先备好），奖品凑不满的才发钱，
            ...
            public bool HasSnapshotObligation { get { return _cashSnapshotRequired; } }
            ...
            public void OnPhysicalSaveSucceeded() { _cashSnapshotRequired = false; }
            ...
            private static ModeHCashBetRecord Decode(string json)
            {
                ...
                if (!BossRushJsonParser.TryParse(json, out root, out error) || ReadSchema(json) != 1) return null;
                ...
                if (record.status < StatusNone || record.status > StatusRefunded || record.amount < 0
                    || record.kind < KindCash || record.kind > KindItems) return null;
            ...
            private static string Encode(ModeHCashBetRecord record)
            {
                ...
                StringBuilder sb = new StringBuilder("{\"schemaVersion\":1,\"runId\":\"");
