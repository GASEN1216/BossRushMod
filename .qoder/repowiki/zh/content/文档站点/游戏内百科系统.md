...
- 提示块：每条 `[tip] text` / `[warn] text` 必须写成单行，遵循 `WikiCalloutSingleLineGuard.py`；不要用跨行标签块。
- 纯文本边界：解析器不支持 Markdown 表格与图片。多列数值改成带标签的列表；在线站配图和速查表由 sync / 主题层添加。
- 链接：支持 [text](url) 与纯 URL，系统将转换为可点击链接。
- 多语言：在 zh/en 目录下分别创建对应 .md 文件，保持内容与结构对应。先按当前生产代码核实触发条件、数值、获取渠道与例外，再同步两种语言；中文正文也不是高于代码的事实源。
- 发布前核对：除了正文，还要检查 `wiki-site/hubs/`、`infobox.mts` 与首页摘要，运行站点构建、导航回归及 Wiki 守卫。历史版本日志保留当时语义，不把旧版规则改写成现状。
...
集成模块销毁时调用 WikiUIManager.Shutdown，关闭界面、退订取消输入、恢复输入并销毁常驻 root；旧实例不留给后继宿主。图片查看器统一使用现有 ModalInputLease 持有暂停，关闭/销毁释放，不直接写 timeScale=0/1。实机仍需 owner 检查关图、卸载和重载后的输入与暂停状态。
