...
- 入场资源回滚走**可持久化欠账账本** `ZombieMode/ZombieModeEntryDebt.cs`（2026-09-12 修 `CR-2026-09-11-019`，`SCHEMA+`）：
  ...
  - 结账**先到账再销账**；销账失败只会在极端情况下重发一次，绝不吞玩家的钱物。邀请函逐张推进，
    中途资源掉线时剩余张数留在账上。
  - 结账点：官方 `EconomyManager.OnEconomyManagerLoaded`（命名方法、幂等订阅，随 `ZombieModeRuntimeModule`
    销毁退订）与每次入场扣款之前。
  - 宿主 partial 只剩两行转发（AGENTS 4.15）；由 `tests/ZombieModeEntryDebtGuard.py`（10 条破坏探针）
    与执行回归 `tests/fixtures/ZombieModeEntryDebt`（58 条断言）守卫。
- 击杀掉落净化星，具有磁吸半径与拾取距离。
- 自动收集超时回收，避免滞留。
