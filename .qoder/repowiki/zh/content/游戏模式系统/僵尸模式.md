...
- 安全区：部署时立即清除区内非 Boss 丧尸并把 Boss 移到边界外，周期性维持丧尸禁入；玩家在当前安全区内直接伤害丧尸会立即取消整个安全区，圆圈、地图 POI、绑定终端、仇恨压制和丧尸禁入同步解除。Combat 阶段可使用一次性便携安全区装置在当前位置替换安全区，替换时清理旧视觉、地图 POI 与绑定临时 NPC；便携安全区跨奖励界面和下一次准备期保留，到下一波正式开始时清理。
...
- 安全区：每波生成普通安全区，玩家进入时抑制威胁；玩家在当前安全区内直接伤害丧尸会立即取消整个安全区。便携安全区跨奖励界面和下一次准备期保留，到下一波正式开始时清理。安全区被手动替换时旧地图标记和绑定服务 NPC 会同步清理。
