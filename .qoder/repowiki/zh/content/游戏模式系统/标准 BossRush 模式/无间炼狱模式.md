...
2026-09-22 审计修复（COMPAT，L1/L2，未实机）：空 Boss 池在开战副作用前拒绝，难度入口保持可重试。标准与无间生成捕获 WavesArenaRuntimeModule 的波次代次、场景 handle 和玩家身份；失败、重试与工厂返回均复核，专属 Boss 透传同一取消门。胜利等待复用此 owner 判定。现金磁铁和累计现金气泡改为取用时解析中英。

里程碑由 `LootAndRewards/InfiniteHellMilestoneDelivery.cs` 唯一状态机分帧交付：每帧最多 8 件，每阶最多 100 皇冠和 100 堆合法现金。指数奖励以 long 饱和计算，超出物理预算的皇冠按真实 prefab 的 `GetTotalRawValue()` 完整标价折现，现金余额经官方 EconomyManager.Add 转入账户；现金池字段只是展示累计，不承担兑现。未交付余额在当前局重试；新局/过图释放该局 owner。改动保留宿主数值范围内的完整标价总价值；普通商店的出售折扣不是本次折现口径。超过账户 long 上限时饱和，不能回绕成负数。回退仅需恢复旧交付服务接线，不涉及存档 schema（但旧算法的溢出会随之回来）。

现金池气泡的 await 返回后也复核原 WavesArena owner，过期续体不会继续操作后继场景的奖励和下一波等待。原 ROOT-U1 的具体重复发奖后果仍未复现，这项按防御加固记录。
