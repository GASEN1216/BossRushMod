...
- **2026-09-23 起玩家看到的流程**：（首次或更新后）加载页「擂台准备中」→ 唯一的选人页，点一位即签主将、自动配接力并开打；
...
- **加载页**：生产认证显示为「擂台准备中」+「正在请选手上台热身（n/总数）」+ 进度条 + 「取消并退票」，不再显示「兼容性诊断 / 认证进度」与内部原因。认证缓存改按游戏 / Mod / 内容三签名命中，不再比对进程内 `slotGeneration`，同一构建只热身一次（Mod DLL 重编译后仍会重跑一次）。
