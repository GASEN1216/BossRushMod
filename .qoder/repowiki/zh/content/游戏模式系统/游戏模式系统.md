...
- 独特机制：
  ...
  - 复用 Mode D 物品池与刷怪点分配，一次性生成所有 Boss。
...
### Mode G：宿命回响
- 进入方式：裸装 + BossRush 船票 + 宿命回响信物（TypeID 500057），且无营旗、无血猎收发器；场内通过专属交互入口打开确认页，预检失败一律 fail-closed 并退还道具。
- 核心规则：固定九波三幕的循环对抗。每局的波次计划在开局时由 runSeed 确定性生成，Active 阶段只读；你的每一次战斗行为（击杀方式、用弹习惯、距离偏好）会被遥测采样，成为下一局反制你的题目。
- 独特机制：
  - 三轴反制：距离回声、弹药点名（禁用上局高威胁弹种）、属性封锁，每波横幅公示当前轴；破解反制可获得 Resolve。
  - 宿敌追猎：第 3/6/9 波为宿敌波；击败你的 Boss 会升格为宿敌（R1-R3）并跨局记忆，复仇击败后结算面板给出宿敌预告与图鉴/印章进度。
  - 宿命契约：入场前从适应/处决/节奏/风格四族中选择契约，达成后铭刻印章并计入契约连胜。
  - 严格奖励事务：胜利奖励 6-10 件，件数由 Resolve 分档（0-2→6 件，逐档至 10-11→10 件），物品按价格分位数分带（<P75 基础带、P75-P95 高级带、>P95 不进槽），事务性一次性发放。
  - 复玩记录层：总场次、胜利数、最佳波次、宿敌图鉴里程碑均为纯荣誉展示，不参与任何数值计算。
- 波次管理：固定节拍 1/2/1宿敌/1/3/1宿敌/1/3/1宿敌，休整普通 8 秒、第 3/6 波后 20 秒；首胜局含 3 个署名 Boss（波 3/6/9），复战局保留 2 署名 + 1 官方 wildcard。
- 奖励系统：胜利结算发放分带奖励；败北不发奖励，但保留宿敌/记录进度，以复仇钩子驱动下一局。

```mermaid
flowchart TD
Entry(["裸装+船票+信物入场"]) --> Confirm["确认页：选择宿命契约"]
