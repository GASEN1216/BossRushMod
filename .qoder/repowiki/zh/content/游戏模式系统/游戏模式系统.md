...
## 简介
本文件系统化梳理 BossRushMod 的八大玩法模式：标准 BossRush（弹指可灭、有点意思、无间炼狱）、Mode D（白手起家）、Mode E（划地为营）、Mode F（血猎追击）、Mode G（宿命回响）、Mode H（百战留痕：黑市鸭王杯）以及僵尸模式。内容覆盖进入方式、核心规则、独特机制、波次管理、敌人生成、难度调节、奖励系统等关键实现，并给出可视化流程图与时序图，帮助不同技术背景的读者快速理解与上手。
...
- 核心规则：四阶段大逃杀（准备→赏金→猎杀风暴→撤离），持续失血；击杀按入场生命回血，最大生命成长封顶于 +50%，溢出成长转为带烧伤和双倍失血风险的命火过载；另有赏金标记追踪与工事系统。
- 独特机制：
...
### Mode H：百战留痕（黑市鸭王杯）
- 进入方式：由擂台门交互进入，走 typed pending entry kind（`BossRushPendingEntryKind.ModeH`），与 Mode G 互斥；Mode H 现属恒开默认内容，旧 `BossRush_ModeHEnabled` 键只做兼容。地图页只展示通过五点位审计的地图，五种拒绝原因各自恰好退还一张预扣船票。
- 核心规则：玩家不是选手而是**经理人**。一季只做一次五席试棚（覆盖五种公开原型、至多一名异常、至少两名稳定底色、展示顺序由 runSeed 固定，关页不重抽），五选二签约，落选三席按固定种子分流为回场签 / 候签 / 撕票。随后打三幕六场，每场最多 180 秒，第 6 场即冠军赛。
- 独特机制：
  - **看懂再押**：敌军计划先于整备冻结（编制骨架 × 进场剧本 × 擂台条件三层），赔率是公开分差 x1–x5，只有 `VerifiedBehavior` 的行为进入分数；每场一次免费侦察只揭示一个冻结字段。
  - **只喊一嗓子**：全场唯一主动操作是拍铃，赛前锁定一条口令、战中只触发一次，6 秒窗口内以 0.1 秒周期重申，压过行为树的定期重发。
  - **赢了也长疤**：五条伤病与八条战痕都必须落在已验证控制点或 Mode H 自结算上，利弊绑定不可拆；战痕最多三条，拒绝换名声。
  - **ERROR 完整互换**：有几率把控制权交到玩家手上，同时选手性格进入玩家身体在看台自行行动。首发唯一允许的 Harmony 新增就是 `CA_ControlOtherCharacter.CanMove/CanRun` 两个 postfix；`CanUseHand`/`CanControlAim` 保持原版 false，因此看台身体在引擎层面就无法用武器、无法瞄准。
  - **真实仓库抵押**：逐场自愿、默认不选、不选也能打完六场；`ModeHWarehouseStakeJournal` 是唯一真实仓库写入者，阶段严格单向，证据不足时优先保护资产。
- 详见：[Mode H：百战留痕（黑市鸭王杯）](file://.qoder/repowiki/zh/content/游戏模式系统/Mode H：百战留痕（黑市鸭王杯）.md)
...
## 结论
BossRushMod 通过统一的波次与配置框架，将八大模式有机整合：标准 BossRush 强调挑战与成长，Mode D 提供从零开始的赌狗体验，Mode E 打造多阵营混战的沙盒乐趣，Mode F 带来高压大逃杀的紧张节奏，Mode G 以跨局宿敌记忆与三轴反制构建“越玩越针对你”的复仇循环，Mode H 把玩家从选手换成经理人、用“看懂再押 + 只喊一嗓子”构建观战向的斗兽赛季，僵尸模式则构建独立的 Roguelite 生存循环。各模式共享刷怪点、掉落与配置体系，既保证了稳定性，又便于扩展与调优。
...
- 经理人向：Mode H，三幕六场的斗兽赛季，只做签约、看盘、下注与一次拍铃，适合喜欢情报博弈与养成叙事、不想全程亲自操作的玩家。
...
## 2026-09-07 F3 九波验收（COMPAT）

`DebugAndTools/F3GameplayValidationSeasons.cs` 的 MODE_G_NINE_WAVES 等每波正式槽位提交，核对登记 Boss 全部活动、存活、敌对，再逐帧通过 Health.Hurt 结算。等待完整九波、生产奖励成功终态及 victory_runId 档案记录；仅 ValidationStartModeG 返回 true 不再足以证明可玩。
测试伤害走 buff/effect 且武器 ID=0，不验证三轴反制、无伤、实际装备计分或战斗难度。没有直接改生命周期、强制解除怪物无敌或直接判胜；真实 AI/操作仍需人工，新流程尚未实机验收。

## 2026-09-11 Mode H 市场与敌军计划接线复核

第 4 场败者市场的资格来源固定为本场实际登记并死亡的最后一名敌军。`ModeHCombatTelemetry` 在死亡事件成功从本场存活敌军集合移除时记录 stable key；结果锁定后到达的迟到或重复死亡事件不能改写该事实。`WriteReport` 只在第 4 场胜利且 stable key 仍通过生产认证时写入已有的 `finalDefeatedProfileSnapshot`、`specialEnemyEligible` 与 `specialEnemySourceTag` 字段。市场构造和接受报价时再次核对场次、胜负、认证来源、当前合同和选手状态，报价卡在接受前确保已有审计档案。

敌军计划生成把合同选手 profile 映射为公开 `archetypeId` 后交给 `ModeHEncounterPlanner` 的 roster veto；敌军池排除本季五席候选与当前合同选手，避免把已签或撕票选手重新作为普通敌军抽回。`tests/fixtures/ModeHMarketAudit` 仅验证源码接线，不连接 Unity、AI、协程和玩家仓库；完整六场与实机市场体验仍需人工 smoke。

计划构造在既有候选次数与技术重试上限内增加确定性组合修复：随机单位组合触发威胁走廊或 roster veto 拒绝时，按 stable key 序枚举有限组合，重新校验人数、必须回响核心、威胁上下界和能力矩阵。枚举只发生在看盘/整备阶段，不进入战斗帧；认证池不足和确无合法组合仍 fail-closed。`ModeHMarketAudit` 的生产源码夹具覆盖第 4 场市场 20 条事实链、100 个 runSeed × 六场 600 个计划及小池/单原型边界，当前 600/600 可构造。

 签约第二名选手前，Mode H 现在会用当前 runSeed、认证敌军池、回响回场核心和现有技术重试规则预构造 1 至 6 场计划。若某个合同组合在当前认证池与威胁走廊下没有完整六场解，签约不会写入 Season，选秀页提示玩家更换替补；该检查只读，不改变押注、物品或经济状态。生产夹具已验证完整池组合可接受，认证池仅剩八名且排除五席导致第 4/6 场无解的组合会在签约前被拒绝。

第 4 场转会接受前同样预构造从下一场到第 6 场的剩余计划；替补更换后若任一场在技术重试预算内无解，报价保持 Pending、合同与资产不变，并提示保留当前阵容。选秀完整性门拒绝后，选秀页会提供“重新选择主将”动作清除待选主将，玩家可重新选择主将与替补，不会被卡在单一主将上。
