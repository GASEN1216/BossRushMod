...
- 悬赏系统：生成初始标记、Boss 间印记继承与成长、玩家击杀回血与生命上限成长、榜首广播、额外高品质掉落。
...
- [ModeFPhases.cs:250-334](file://ModeF/ModeFPhases.cs#L250-L334)
...
- 玩家击杀：击杀带印记 Boss 为玩家加 1 印记，同时回血（普通 50%，悬赏额外 25%）与最大生命成长（普通 1，悬赏取最大值）。
...
- 奖励气泡：击杀回报（血量、生命上限、悬赏印记）以对话气泡形式展示。
...
- 内存管理：活动工事硬上限（24），退出时清理所有 Boss、工事、商人、快递员与龙息 Buff 处理器，释放提取区域与 UI 对象。

**章节来源**
- [ModeFPhases.cs:96-162](file://ModeF/ModeFPhases.cs#L96-L162)
...
- [ModeFPhases.cs:453-579](file://ModeF/ModeFPhases.cs#L453-L579)
...
- [ModeFPhases.cs:228-245](file://ModeF/ModeFPhases.cs#L228-L245)
