...
F3 仍要求登记敌人同时激活、存活且敌对，第三轮首波三只狼已通过这些检查。
多波用例在第二次死亡期间另抛格式异常；已安装的击杀提示在同帧连杀时可能解析尚未初始化的经验文本。
F3 当前波快照改为每次真实击杀后让出一帧，异常输出完整栈并 FAIL，推进后仍等新波完成生成。
这只调整 Dev 验收节奏，生产规则不变；第四轮多波已实机通过，wave=1->2、playable=3/3。
...
官方小怪 preset 可能属于中立阵营。Mode D 在把角色加入本波登记表前必须设置并回读 runtime team，
确认其确实对玩家敌对；AI target 只能决定追踪目标，不能替代伤害与敌对判定。修正失败的角色直接
销毁，不进入波次计数。结束模式会注销敌人恢复、禁掉落并销毁已登记实体；即使模式 active 标记已
提前清除，重复结束仍执行残留清理。
