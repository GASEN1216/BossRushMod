...
2026-09-22 异步生成审计修复（COMPAT）：每次创建捕获宿主、场景 handle、槽号和 `deathWraithSpawnGeneration`；恢复物品树、创建角色、恢复近战及战斗配装之后均复核。配置关闭、场景清理和 `OnSetFile`（含同槽重载）使旧代次失效，旧 finally 只释放自己仍拥有的预约，不能清除后继请求。未移交的角色由创建 owner 回收，角色尚未创建时回收完整物品树；装备恢复抛异常同样收尾。`AuditCombatSeptember/Wraith` 在五个等待点逐一重载并创建相同 raidID 的新请求，证明迟到结果与预约隔离，另覆盖 factory / 装备恢复异常。验证是内存替身 L2，未访问玩家存档或启动游戏；下文旧快照按当前源码与本段解释读取。
