...
- UI管理器（ReforgeUIManager）
  ...
  - 属性条目点两下锁定（第一下待确认、2 秒内再点才扣冷淬液），可固定行左侧 Accent 细条、成功后数值金色（2026-09-23 起，见文末）
  - 对比显示预制体与当前值的差异
...
- 与UI联动：点两下属性条目（第一下只进入待确认）消耗冷淬液并固定，数值变为金色
...
- 概率显示：UpdateProbabilityDisplay → RenderReforgeCostText（`ReforgeUIManager_Feel.cs`），主信息「重铸幅度」「总计花费」两行 + 三行次信息（2026-09-23 起）
...
CanExecuteReforge 与普通重铸按钮共用未固定属性判据；全锁物品不会扣款。执行返回 Success=false 且没有实际属性变动时退还原币种；官方支付先变余额再通知，通知抛错按实际扣款补偿，退款通知异常也在 finally 释放 busy 并刷新余额。已应用的部分变动不作为免费重铸。普通费用、概率、极性与按钮文本在刷新时走 L10n。NpcAuditFixes 覆盖三种属性、金币/净化点、失败退款与监听抛错。

## 2026-09-23 审美整改（COMPAT，未实机）

- 费用区：从七行公式 + 霓虹色富文本收成「重铸幅度 72%」（30 号，按档位 SuccessText / WarningText / DangerText）与「总计花费」两行主信息，外加三行 16 号次信息（品质系数·价值系数、投入加成·极性费用、正向·负向概率）；「0.20×a×b+c」算式不再展示；钱不够只染「总计」一行并写「还差 X」。富文本色统一取 `IntegrationUIFeedback.*Hex`。
- 属性固定：第一下进入待确认（行底 Accent 淡色 + 「再点一次固定」+ `UI/pop`），2 秒内再点同一行才走原有「检查 → 扣冷淬液 → LockProperty」路径；超时、换行、切换物品、关闭界面都撤回。可固定行常态有左侧细条，冷淬液计数下方有说明行。只给数值 TMP 上色，清理时还原官方原色（不再刷成白色）。扣费 / 退费逻辑未改。
- 重铸结果：变化的行逐行揭晓（行底闪涨 / 跌 / 触顶色，数值 1.15 倍回落），触顶行金色「Max」+ `UI/pop`；词缀随机后未锁行依次淡入、描边短暂换稀有度色，抽到稀有播 `UI/level_up`。
- 其它：冷淬液计数 20 号 token 色、图标取不到时退官方物品图标或整格隐藏（不再用「◇」「◆」）；词缀锁定按钮走共享次级按钮，已锁为 Warning 底 + 传说金描边。新增 partial `ReforgeUIManager_Feel.cs` 与守卫 `tests/ReforgeUIFeelGuard.py`。
