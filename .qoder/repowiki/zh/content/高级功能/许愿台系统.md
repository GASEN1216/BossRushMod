...
### 动画效果与 UI 反馈
- 建筑粒子：星尘上升（亮核光点、加色亮档，0.07–0.12 m 随机）与星芒闪烁（四主芒星芒、加色热档，0.14–0.2 m、随机旋转）。材质取自共享特效层 `BossRushFxKit.GetShapeMaterial`，原色相走粒子顶点色；不再自造圆点贴图和把颜色翻倍的 `_TintColor` 材质，取不到材质时关掉粒子渲染器（2026-09-23 特效审美审查 VA-15）。缺包占位模型改用官方 SodaCharacter 着色器（`BuildingModelHelper.CreateOfficialTintMaterial`，写 `_Tint`）。
- 奖励气泡：通过 DialogueBubblesManager 显示冷却与结果提示。
- UI 面板：固定高度多行输入框、滚动条、字数统计、匿名开关、弹幕展示与状态提示。
- 2026-09-23 审美整改（COMPAT，未实机）：面板配色全走 token、去掉 `UI.Shadow` 硬投影与框里套框（新 partial `WishFountainUI_Feel.cs`）；许愿成功停 0.6 秒（描边变绿 + `UI/confirm`）再用官方 `FadeGroup.Hide()` 淡出，抽奖只在关闭出口开一次。弹幕只在上下各 25% 两条带里飘，三层景深，字底改 TMP `UNDERLAY_ON` 共享材质。抽奖揭晓（新 partial `WishFountainRewardAnimationView_Reveal.cs`）：轮带 1-(1-t)⁴ 单曲线减速、`RectMask2D` 软边、格子圆角 + 品质色描边；品质色取官方 `GameplayDataSettings.UIStyle.GetDisplayQualityLook(...).shadowColor`（取不到退 `Rarity*`）；停轮后中奖格弹一下 + 品质色柔光 + 轮带下方奖品名横幅，Q≥5 淡闪白，音效 pop / level_up / special.mp3。滚动中按 Esc 只跳到揭晓，揭晓后再按 Esc / 点击才收下（奖励仍只在 `Complete()` 发一次，守卫 `tests/WishRewardRevealGuard.py`）。收场头顶的奖品名气泡暂保留（与横幅重复，待 owner 定）。
...
- [WishFountainFetchPipeline.cs:231-249](file://Integration/WishFountain/WishFountainFetchPipeline.cs#L231-L249)
- [WishFountainFetchPipeline.cs:452-569](file://Integration/WishFountain/WishFountainFetchPipeline.cs#L452-L569)
## 2026-09-06 通用建筑模型工具抽取

`SAFE`。`CollectStarwishRenderableComponents`、`TryGetCombinedBounds`、`AddStarwishGraphicsCollider`、`FixStarwishModelShaders` 的原实现移至 `Common/Buildings/BuildingModelHelper.cs`，许愿台原私有方法保留薄转发。这些工具不再通过 ModBehaviour partial 被报箱隐式共享；报箱模块直接调用共同实现。材质回退顺序、排除粒子 Renderer、包围盒与碰撞体计算均保持原代码。

许愿台模型的加载、静态缓存、卸载继续由原许愿台生命周期管理。报箱仅经其明确的 ModBehaviour owner 读取现有 prefab，无第二份 AssetBundle 加载或资源所有权。
