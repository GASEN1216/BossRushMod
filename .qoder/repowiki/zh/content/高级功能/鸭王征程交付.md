...
`CampaignGuideTable` 定义 590201–590214 十四条一次性引导，覆盖 Mode D–H、丧尸、遗种、随机事件、天空岛 Boss 装备、菜地、陈列、词缀、重铸与日报。仍由同一 CampaignOfficialQuestClient 注册到共享 OfficialQuestProjection，不增加补丁 owner，不改六章任务身份。玩家入口是基地 Jeff 的官方任务页。2026-09-25 起一条接一条发放（`CampaignGuideTable.NextOfferableId`）：手上有接了未交付的就不挂新的；否则挂表内第一条未交付且前置满足的——菜地要第一章、陈列要第二章的设施 token（`CampaignOfficialQuestClient.GuidePrerequisiteMet`），前置未到先跳过不挡后面。旧档已同时接下的照常保留。文案改为杰夫当面对玩家说的话。执行回归 ContentTransactions 覆盖首发、跳过、单条进行中与交付后接续。
