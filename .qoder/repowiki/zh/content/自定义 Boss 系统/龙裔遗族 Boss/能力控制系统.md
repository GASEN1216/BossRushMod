...
- 燃烧弹资源接线（2026-09-17，COMPAT）：`ItemAssetsCollection.GetPrefab(941)` 校验 `TypeID` 与原始名称键 `Item_FireGrenade`，再取 `ItemSetting_Skill.Skill as Skill_Grenade` 的 `grenadePfb`；缓存技能引用，投掷前按官方 `Skill_Grenade.OnRelease` 同步引信、爆炸范围等参数。禁止按 `fire` 子串、爆炸特效或已加载手雷顺序选择，`Firework` 会误命中。资源缺失或身份失配只告警并走既有后备；定时投掷与复活八方向共用创建入口。场景切换仍由 `ClearStaticCache` 清空。物品依据：[燃烧弹 #941](https://escapefromduckov.net/zh/items/firegrenade)。
...
- 对象池与预制体缓存：
  ...
  - 燃烧弹技能在 Initialize 时按官方物品 ID 一次性获取并缓存，子弹仍从 Boss 枪械预缓存；燃烧弹不再扫描全局资源
- 事件订阅管理：
