...
- 资源身份：固定读取官方燃烧弹 #941（`Item_FireGrenade`）的 `ItemSetting_Skill → Skill_Grenade → grenadePfb`；官方技能同时提供引信和爆炸范围，创建后、Launch 前同步。不再用 `fire` 模糊匹配（会误选 `Firework` 烟花），不接受任意手雷后备。定时投掷和复活八方向都经过这条路径；`DamageInfo` 的 30 点赋值、投掷间隔和抛物线仍沿用龙裔设定；官方技能关闭直接爆炸，实际地面持续伤害由原版火区资源定义。
