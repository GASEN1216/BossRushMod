...
2026-09-22 审计修复（COMPAT）：龙皇生成器在各等待点复核原调用者的有效性；取消回收按具体角色摘掉套装、死亡 / 掉落委托与实例登记。初始化异常只释放本次实际增加的 AssetBundle 引用，避免减少另一只龙皇持有的引用。孩儿护我请求由父控制器代次拥有，超时、协程 Dispose 和父清理都会作废请求，迟到结果由生产者与消费者两层回收。Mode G 子龙裔先以 `PhaseProxy` 提交给本局，再激活完整能力；父 owner 清理释放 handle 与辅助登记，禁止 Legacy 掉落。`AuditCombatSeptember/Child` 覆盖正常接管、超时 / Dispose 迟到结果、提交先于激活及取消 prepare；结构守卫另钉住生产接线，尚无 L3。
...
- 性能监控
  - 场景切换问题走模块统一的 `OnSceneLoaded(SceneRuntimeContext)` 钩子与 dev 构建 DevLog 定位；契约级失败由 `CriticalLog` 在正式构建下也上报。
...
- 调试与测试
  - 用 `compile_dev.bat` 出 dev 构建观察场景切换与传送的 DevLog。
...
## 2026-09-07 死亡后迟到受伤事件（COMPAT）

官方 `Health.Hurt` 的死亡处理可能先让 Mode G 停用龙王，随后才调用 `OnBossHurt`。受伤入口现先检查组件启用、角色/生命存在、IsDead 与 Dead 阶段，已失效实例不再回血或重启孩儿护我协程。活体 1HP 召唤的幂等判定和转阶段保护保持原行为。

章节来源：`Integration/DragonKing/DragonKingAbilityController_AttackFlow.cs`、`tests/DragonKingChildSpawnCancellationGuard.py`、`tests/fixtures/GameplayLogFixes/README.md`。Player.log 已证实旧路径，修复后的 Unity 战斗仍待复测。
