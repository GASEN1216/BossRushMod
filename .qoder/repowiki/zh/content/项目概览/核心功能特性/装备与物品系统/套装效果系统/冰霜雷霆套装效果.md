...
> 2026-09-25 本机作者源复核（OPERATIONAL）：当前仓库包符合下述已发布校准，但作者工程的校准 JSON、霜冠 prefab 和同包铠甲 prefab 仍是旧值。现已同步并仅重打 `frost_set`，Unity 校验通过，所有包内对象、变换与整体 SHA-256 均与仓库包相等；作者导出与 ResourceRelease 同步完成。本轮无游戏部署或试戴，不能据此认定实机穿模已验证。

> 2026-09-23 尺寸复核（OPERATIONAL）：Unity prefab 已调好但发布目录仍是旧 `frost_set`，导致 owner 复测比例未变。本次沿 `HelmetFitBundleBuilder` 单独重打并回读：霜冠子节点 scale `[24,30,24]`、y `0.29`，寒冰铠甲 scale `[58,64,38]`、y `-0.035`；运行时护甲仍统一乘 `4/3`。霜冠完整轮廓宽约 `0.458 m`；冠环贴合和动作穿插需实机确认。新包 SHA-256 为 `a323666417cd170f1941f5d616ec0750442426990aeae80ae52879b64fa4e553`，部署必须核对作者发布源、仓库和实际游戏路径，不能只确认 prefab。校准表入口：[helmet_fit_profiles.json](file://tools/helmet_fit_profiles.json)。

> 2026-09-22 更新（COMPAT）：普攻附带现为 `FrostBiteStep` / `ThunderBiteStep`。延时结束先验激活代数，再释放当前 pending，之后复核冷却；旧请求不能释放新激活的排队标记。完整当前说明见 [套装效果系统](file://.qoder/repowiki/zh/content/装备与物品系统/套装效果系统.md)。下文带 2026-09-07 日期的 `ThunderChainStep` 是旧击杀触发实现的历史说明。

## 2026-09-19 人工实测修复（COMPAT）

当前定位为元素防护与辅助反制，输出低于龙裔/龙王套装。四件最大耐久均为 100。冰葬累计 3 次主角直接击杀后，对尸体 3 米内最多 3 个敌人造成 8 冰伤并冻结，6 秒冷却；引雷术累计 3 次直接击杀后，对尸体 4 米内最多 2 个敌人各造成 12 电伤，5 秒冷却，只有一跳。雷霆反震为 3 米/12 电伤/5 秒冷却，25% 概率与近身 6 米触发条件保持。持续伤害、套装自建伤害和无效目标不累计；死亡、卸装与切图重置累计。

`FrostThunderSetConfig.ApplyModelFit` 在视觉子层幂等修正：护甲放大为原来的 4/3，两顶头盔追加 Y 轴 180 度旋转。官方穿戴逻辑会清零 agent 根旋转，所以不能把翻转只写在根节点。GroundPoint 等无渲染定位点不跟着旋转。

套装不是普通命中触发：基地打人偶不会验证击杀技能；元素回补需要相应元素的实际受伤，冻结/反震需要玩家被近身攻击者打中。`SetBonusCoroutines` 执行了直接击杀累计、冷却、伤害/半径/目标上限、死亡复位和雷击不续跳回归（L2）。比例、朝向和实际冻结表现仍需 owner 实机确认（L3）。下文 2026-09-06 数值说明属于旧基线，以本节与代码为准。
...
> - 表现层门面在 `SetBonusVisuals.cs`：眼光复用龙套装头骨查找、表现在 `SetBonusFx.cs`（HDR 眼点 + 弱补光）；爆发环转调 `NewWeaponFx.PlayBurst`（2026-09-23 起与新武器同一份实现）；电弧为 `SetBonusArcPool`（外晕 + 芯两层）；音效由 `tools/gen_setbonus_sfx.py` 合成。
...
## 2026-09-11 死亡后的受击入口（COMPAT）

官方 `Health.Hurt` 会在同一致死调用中先触发 `OnDead`，禁用对象，再触发 `OnHurt`。冰霜套与雷霆套现在在受击入口检查 `health.IsDead`，防止迟到事件再次冻结攻击者、改写冷却或用死亡后的新 `setBonusGeneration` 排队反震。死亡分派器仍负责作废已排队的旧技能；两处门控共同保证死亡后不会重新武装反制。

来源：`Integration/Bonus/FrostSetBonus.cs`、`Integration/Bonus/ThunderSetBonus.cs`，`CR-2026-09-11-014`。存活时受击反制和击杀技能保留，真实复活与战斗表现仍需游戏内确认。

## 2026-09-23 霜冠缩小、寒冰铠甲加大（资源校准）

owner 2026-09-22 截图（image-13）：霜冠约为头宽 2.1 倍，铠甲下缘露出肚子。改动如下。

**霜冠**
- 校准表 `tools/helmet_fit_profiles.json` 改为 scale `[24,30,24]`、position.y 0.29，朝向不变。
- 包内实测 0.458 × 0.426 × 0.396 m，比官方鸭头最宽处 0.49 小；冠圈贴在头顶。

**寒冰铠甲**
- 作者工程 prefab 子节点改为 scale `(58,64,38)`、position.y −0.035。运行时 `ApplyModelFit` 的 4/3 保留。
- 穿戴后宽约 1.48、深约 0.72、高约 0.84 m，两侧下缘约到 y −0.40，盖住腹部。

**打包与校验**
- 经 `HelmetFitBundleBuilder.ApplyAndBuildAllAndExit` 重打，只有 `frost_set` 变化，thunder_set 与 skyisland_boss_gear 哈希不变。
- UnityPy 从包内读回两个子节点的变换，与上面的值一致。
- 注意：护甲没有校准表，数值只在 prefab 里。雷霆护甲沿用原值。
