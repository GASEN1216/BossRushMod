...
## 简介
本模块为 BossRushMod 的游戏模式系统，提供八大玩法模式的统一入口、规则与运行机制。涵盖标准 BossRush（弹指可灭、有点意思、无间炼狱）的波次管理；Mode D 白手起家的独立敌池与随机装备成长；Mode E 划地为营的多阵营沙盒混战与商人经济；Mode F 血猎追击的四阶段状态机、赏金系统与工事系统；Mode G 宿命回响的跨局宿敌记忆与三轴反制；Mode H 百战留痕（黑市鸭王杯）的经理人视角赛季——五席试棚、公开分差赔率、每场一次拍铃、伤病战痕继承与真实仓库抵押；以及僵尸模式的独立生存机制、污染系统与净化点经济。文档从进入方式、核心规则、难度调节与特色功能四个维度展开，并给出可视化流程图与时序图，帮助玩家理解不同模式的策略选择与配置项。
...
- ModeG：宿命回响模式，跨局宿敌记忆、九波三幕与三轴反制。
- ModeH：百战留痕（黑市鸭王杯），经理人视角的三幕六场斗兽赛季，详见 [Mode H：百战留痕（黑市鸭王杯）](file://.qoder/repowiki/zh/content/游戏模式系统/Mode H：百战留痕（黑市鸭王杯）.md)。
...
## 依赖关系分析
- 模式间互斥：Mode D/E/F/G/H 与僵尸模式相互排斥，避免状态污染；Mode G 与 Mode H 另各自提供只读运行时门（`ModeGRuntimeGates` / `ModeHRuntimeGates`）供旧入口查询。
...
## 结论
BossRushMod 的模式系统通过清晰的职责划分与共享能力，实现了标准 BossRush、Mode D/E/F/G/H 与僵尸模式的差异化体验。各模式在波次管理、敌池设计、经济系统与状态机方面各有特色，同时兼顾性能与稳定性。玩家可根据自身偏好选择合适模式，并通过配置项与策略组合获得最佳体验。
