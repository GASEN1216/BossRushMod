...
- 波次控制器：维护准备期/战斗期/结算期，周期性刷怪、Boss 波、压力目标、批量生成与间隔控制；下一波启动时清理普通散落物，保留奖励选择和休息期间的拾取窗口。
- 安全区系统：部署时立即清除区内非 Boss 丧尸并把 Boss 移到边界外，通过生成回调和周期 Tick 持续维持丧尸禁入；玩家在当前安全区内直接伤害丧尸会立即取消安全区，解除仇恨压制和丧尸禁入。便携安全区按阶段分流：战斗期使用替换主槽安全区、本波结束后恢复为带商人的正常安全区，准备期使用则额外部署不带商人的副槽与正常区并存，两槽在下一波开始时一起清除。奖励界面默认只显示当前休息时长和 Edit，点击后才展开 15～300 秒的编辑器，初始默认 45 秒且玩家修改值会跨波次沿用。
...
- 保护范围：部署瞬间清除区内非 Boss 丧尸，Boss 移出边界；边界外保留排斥缓冲，将进入缓冲的敌人持续移到边界外。
- 仇恨规则：玩家未在安全区内直接伤害丧尸时抑制仇恨与追踪；直接伤害丧尸会立即取消安全区。
- 限制规则：单纯开枪、装填、投掷、误伤非丧尸目标或区外攻击不取消；区内直接伤害丧尸在战斗和准备阶段都立即取消安全区。
...
Suppress --> Attack{"是否在区内直接伤害丧尸?"}
Attack --> |是| Cancel["取消安全区"]
Attack --> |否| Keep["保持抑制"]
...
- 安全区异常：验证玩家位置判断、威胁抑制、攻击取消和下一波清理逻辑。
...
- 后期建设：在 Boss 波前最大化净化点储备，选择高价值奖励节点；使用安全区时避免在区内直接攻击丧尸。
