...
- 安全区系统：部署时立即清除区内非 Boss 丧尸并把 Boss 移到边界外，通过生成回调和周期 Tick 持续维持丧尸禁入；玩家在当前安全区内直接伤害丧尸会立即取消安全区，解除仇恨压制和丧尸禁入。便携安全区跨奖励界面和下一次准备期保留，到下一波正式开始时清理。奖励界面默认只显示当前休息时长和 Edit，点击后才展开 15～300 秒的编辑器，初始默认 45 秒且玩家修改值会跨波次沿用。
