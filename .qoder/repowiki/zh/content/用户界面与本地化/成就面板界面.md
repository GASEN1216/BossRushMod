...
- 列表渲染与排序
  - 获取所有成就后执行排序：可领取的最前（2026-09-24 A-23），其次已完成、隐藏成就后置，同组内按分类枚举值与难度评级排序。
...
- 视觉更新
  ...
  - 累计类成就（击杀数/通关次数）未完成时，右侧状态位显示「12 / 50」+ 细进度条（Filled + 纯色 sprite），描述正文不再括进度（A-24）；
    已领取不再留灰按钮，同一格写「√ 已领取」（A-21）；没有可领的时候页脚不挂「一键领取」（A-22）。
  - 打开时占模态租约（ClaimModalInput），ESC / 手柄取消走 PetNestCancelKey 并用掉官方取消事件（A-43）。
- 图标加载
...
`Achievement/AchievementView.cs` 改用共享逻辑视口计算面板尺寸，避免 720p 下先按物理像素缩一次、再被 CanvasScaler 缩一次。`Achievement/SteamAchievementPopup.cs` 使用 400×100 提示框，标题和两行描述各有独立区域并受省略边界约束，长英文不再横向穿出背景；保留既有图标、稀有光效与堆叠动画。成就条件、奖励和输入入口不变。实际长文本和多弹窗叠放待验。

## 2026-09-23 审美整改（COMPAT，未实机）

- 面板：配色全改共享 token；页头 / 统计栏 / 页脚不再铺全宽直角底色，改用分隔线（`ZombieModeUIHelper.CreateSeparator`），标题白字 28 号，前面一根传说金竖条；遮罩走 `BossRushUI.CreateBackdrop`（暗角 + 淡入）；打开走 `BossRushUI.PlayOpenAnimation`，关闭 0.12 秒淡出后才 `SetActive(false)`（`IntegrationUIFeedback.FadeOutAndDeactivate`）。
- 一键领取：官方按钮 prefab 只切 `interactable`，不再写 `Image.color`；按钮文字带可领数量「一键领取 (N)」。回退按钮走共享 `CreateButton`，AccentFill 底。
- 条目：一层 Card 底 + 共享描边，描边颜色表达状态（待领取 = 传说金，其余 Stroke），不再「外层色块当边框 + 内层再套一张卡」；领取按钮走共享次级按钮配金字；领取成功播 `UI/sell`，金额数字从 1.2 倍回弹、描边从金色淡回 Stroke；图标取不到时去掉图标列、文字贴左，不再画灰方块。
- 解锁弹窗：400×108 圆角卡片（`ApplyFramedPanelSkin`），标题 18 / 说明 15 号；图标框是圆角凹槽 + 描边环（普通 Stroke、稀有传说金）；让位改成 0.25 秒 EaseOut 补间（与帧率无关），退场改为原地淡出 + 向右滑开；过暂停门；弹窗与子物体 `SetParent(..., false)`，高分屏下不再按 1/scaleFactor 缩小。
