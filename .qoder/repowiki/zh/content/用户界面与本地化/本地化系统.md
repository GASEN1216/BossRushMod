...
- 语言切换：玩家已可在游戏中切换语言。文案在取用时按当前语言解析；已缓存的显示名和本地化注入由相应 owner 刷新，不把语言判断只放在初始化。
...
## 2026-09-17 玩家文案审核（COMPAT / SAFE）

本轮从正式编译清单扫描文本候选，再手工核对 NPC、征程、装备、入场说明与天空岛物品的实际消费入口。把长句拆为动作、条件与结果；数值、来源和限制保留，中英文同步。阿稳、新手首遇、羽织与叮当的故事、晴禾与苇白的日常，以及小满的英文覆盖均已更新。

天空岛沿用官方逐句对话。有业务且超过两句时，先读两句即可办事或告辞，选择「再聊聊」才继续未读内容。装备和物品说明用短段落区分风味、效果与来源，不以删掉条件来缩字。

静态字数与隔离回归不能证明实机无截断。具体范围、参考资料及中英文人工检查见 `docs/reports/reviews/2026-09-17_玩家文案审核.md`。
