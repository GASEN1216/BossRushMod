...
**地图选择器逐图进场（2026-09-25）**：6/7 之后按 `ModBehaviour.GetAllMapConfigs()`（选择器的同一份清单，当前 9 张，含风暴区地下、
冷库两个子场景）逐张走玩家的路：在基地排好入场（`ValidationPlanMapTourEntry`：同一个待处理条目索引、`bossRushArenaPlanned`、
信标索引，直传来源不扣船票）→ 加载 `sceneID` → 等 BossRush 接管 → 核对 → 官方 `LoadBaseScene` 回基地。每张记 `MAP_TOUR_<场景名大写>`，
全部跑完记 `MAP_TOUR_ALL`。核对项：到达的场景、竞技场激活且没误进别的模式、路牌 / 交互点、刷新点全数加载且每点
能落地、离可走导航 ≤3 m、玩家离配置传送点 ≤4 m 且脚下有地、回基地后竞技场与模式状态清掉。只读，不刷怪、不开波次。
判据在 `F3GameplayValidationMapTourJudges.cs`（纯函数，执行回归 `F3MapTourJudges`），取数在 `F3GameplayValidationMapTour.cs`；
`MAP_TOUR_*` 在游戏侧 `Coverage.Expand` 与 `tools/gameplay_coverage.py` 按同一口径展开。
...
### 全自动实机验收（2026-09-14，Dev 构建）

「自动验收 + 完整待测清单」在主套件回到基地之后多跑一段后半程（`F3GameplayValidationAutotest.cs`），入口仍是这一个按钮：

1. 读步骤表 `Assets/Data/SkyIslandAutotest.json`（加步骤只改数据），跑基地侧步骤；
2. 给专用测试档的天空岛剧情、官方图鉴镜像、岛上物品件数、钱、语言、强制夜里与时间流速存快照（存档键 `BossRush_Validation_AutotestSnapshot_v1`，另落一份 `story_snapshot.json`）；
3. 经基地船点真实出发（船点子场景没加载时按官方缓存位置 `LoadAndTeleport`，都不行才退回生产 `Enter` 并记 SKIP），看门狗 150 秒；
4. 岛上：落地步骤 → 岛内只读套件、收尾快照与 Dev 演练原样复用 → 真实存档状态下的步骤 → 清空成新档按阶段推进（新档 → 双航标 → 打过噬风未敲钟 → 钟守放行 → 结局 → 结局 + 七盏风晶灯），每阶段一轮步骤 → 换语言复拍；
5. 走码头撤离圈回基地（12 秒不起读条就派发返航兜底），还原剧情并读回核对 → 复位环境 → 收回多出来的岛上物品、记金钱 → 基地侧检查。

- **动作库**（`F3GameplayValidationAutotestActions.cs`）只走生产入口与官方公开 API：瞬移先核地面再 `SetPosition`；交互走 `CharacterMainControl.Interact` 与真实读条；面板选项点 `Button.onClick`；官方对话用 `DialogueUI.Confirm` 与选项点击；清场以玩家为伤害来源走 `Health.Hurt`；夜里用 `DevForceNight`；帧时间与 `SkyIslandFrameProfile` 分项计时同 `SKY_PERF_*`。断言求值在 `…Asserts.cs`，名单与纯判据在 `…Judges.cs`；头目 / 岛主相关的等 Boss 刷出（`wait_boss`）、压到血线触发相位机制（`boss_hurt`，噬风沿用 `echo_hurt`）、在场断言（`boss_alive`）、单独击杀后读官方尸体箱核对「配装即掉落」（`loot_boss`，判据 `F3AutotestJudges.JudgeBossDrop`）、瞬移到 Boss 身边（`approach_boss`，截信人的劫包要贴身）与给听雨人记枪数（`feed_shots:need`，调它 `#if BOSSRUSH_DEV` 的 `DevFeedShots` 补到 `ShotsNeeded`，代替真开枪；步骤表只许写 `need`——玩家戴静听耳罩时要的枪数翻倍，写死数字会跟不上规则）在 `…Bosses.cs`；种类名是档案 id 的小写，断风三游猎分 `windhunter_chaser` / `windhunter_stalker` / `windhunter_warden`。步骤表覆盖人工清单 2.10–2.19 与 `GameplayCoverage.json` 的全部 `M_SKY_ISLAND_*`。每一步收尾还比一次会话的 `RescueCount`：一步之内被捞回落脚点 ≥3 次记 `assert:footing` 红（落点站不住会让人原地弹球，以前只写在 `Player.log` 里）。
- **截图**（`…Capture.cs`）：先确认 F3 菜单等调试浮层不在屏幕上，帧末 `ScreenCapture.CaptureScreenshotAsTexture` 取像素；在线性色彩空间算文字对比度（WCAG）、TMP 溢出与省略号截断、世界物体（云蚋、采集光斑、灶火）的可见度。撤离环与回响预警圈不量取样框亮度（框里是日照石面，环被铺面盖住也会过），而是沿 LineRenderer 顶点在屏幕上取环带与内外两侧的线性色，逐段判「环带比两侧更偏向环色」，得覆盖率（纯判据 `JudgeRingCoverage`，屏上不足 16 段记 SKIP）。
  2026-09-15 第五轮复核后又收紧了四处：世界物体投影框大半在画面外、或长边不足 24 px（810×540 下的云蚋）记 SKIP，不判红也不判绿；采集光斑同时认色度偏移（`ChromaShift` ≥ 0.12，白天暖色光斑压在砂岩上亮度差很小）；字直接坐在小底板上（ESC 键帽）时对比度外圈夹在底板内；设计上封顶两行的字幕只排出一行就被省略号截断时判红（`truncated_early`）。截图 metrics 带 `at_ms`（本步内的时刻，连拍判时长用）；`rect` 是 Unity 屏幕坐标（左下原点），`autotest_review.py` 会补一份左上原点的 y。
- **取景与对话时机**：`teleport_view:目标:右:上[:等待]` 按相机水平朝向把目标（标记或建好的采集点 `SkyIslandGather_*`）摆到画面中心右 / 上若干米；`ring_replay:环物体名` 让撤离环停用再启用、重播 0.35 秒开环，接着连拍；`wait_dialogue_typed:秒` 等官方对话这一句打完（私有字段 `continueIndicator`）或选项在等玩家选，只等不点、再等一帧才截；`assert:dialogue_line_contains` 判当前这句含关键字。步骤表守卫要求官方对话截图与单次推进之前必须先有 `wait_dialogue_typed`。UI 存 PNG、世界存 JPG，单轮 ≤300 MB，用量上去后降级，满了只分析不落盘。不用 Steam 截图（依赖 overlay、文件不在结果目录、进程内拿不到像素）。
- **结果目录** `BossRushTestReports/<runId>/`：`manifest.json`（schema `bossrush-autotest-manifest/1`，每步的清单编号、阶段、位置、动作、断言与 metrics、截图文件名）、`summary.md`（还原失败时红字置顶，附覆盖表）、`shots/`；原有 `.log` 与 `.coverage.md` 不变。AI 用 `python tools/autotest_review.py <结果目录> --auto-previous` 生成按清单编号分组的缩略图、红项置顶与上一轮对比。
- **安全**：全部代码整份 `#if BOSSRUSH_DEV`；任何写入先过 `AutotestWriteAllowed`（Dev + 专用测试档 + 这一轮正在跑 + 槽位没换 + 快照已落盘）；还原两道（收尾阶段不被取消打断 + `CompleteSession` 同步兜底），都复位语言、强制夜里、无敌、血量与时间流速；中途崩溃后回基地按快照键恢复。每步在 Player.log 写 `[AUTOTEST] step=… begin / end result=…`。
- **守卫与回归**：`F3AutotestOrchestratorGuard`（编排纪律）、`SkyIslandAutotestTableGuard`（步骤表名字对得上生产、清单逐行归类）、执行回归 `F3AutotestJudges`；换回正式构建后 `python tools/check_dll_identifiers.py --expect absent` 实查 DLL。

来源：
- [F3GameplayValidationAutotest.cs](file://DebugAndTools/F3GameplayValidationAutotest.cs)
- [F3GameplayValidationAutotestActions.cs](file://DebugAndTools/F3GameplayValidationAutotestActions.cs)
- [F3GameplayValidationAutotestAsserts.cs](file://DebugAndTools/F3GameplayValidationAutotestAsserts.cs)
- [F3GameplayValidationAutotestBosses.cs](file://DebugAndTools/F3GameplayValidationAutotestBosses.cs)
- [F3GameplayValidationAutotestCapture.cs](file://DebugAndTools/F3GameplayValidationAutotestCapture.cs)
- [F3GameplayValidationAutotestStory.cs](file://DebugAndTools/F3GameplayValidationAutotestStory.cs)
- [F3GameplayValidationAutotestJudges.cs](file://DebugAndTools/F3GameplayValidationAutotestJudges.cs)
- [SkyIslandSessionAutotest.cs](file://DebugAndTools/SkyIsland/SkyIslandSessionAutotest.cs)
- [SkyIslandStoryServiceAutotest.cs](file://DebugAndTools/SkyIsland/SkyIslandStoryServiceAutotest.cs)
- [SkyIslandAutotest.json](file://Assets/Data/SkyIslandAutotest.json)
