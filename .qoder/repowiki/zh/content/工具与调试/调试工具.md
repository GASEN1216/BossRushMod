...
- **截图**（`…Capture.cs`）：先确认 F3 菜单等调试浮层不在屏幕上，帧末 `ScreenCapture.CaptureScreenshotAsTexture` 取像素；在线性色彩空间算文字对比度（WCAG）、TMP 溢出与省略号截断、世界物体（云蚋、撤离环、采集光斑、回响预警圈、灶火）的可见度。UI 存 PNG、世界存 JPG，单轮 ≤300 MB，用量上去后降级，满了只分析不落盘。不用 Steam 截图（依赖 overlay、文件不在结果目录、进程内拿不到像素）。
