...
- **动作库**（`F3GameplayValidationAutotestActions.cs`）只走生产入口与官方公开 API：瞬移先核地面再 `SetPosition`；交互走 `CharacterMainControl.Interact` 与真实读条；面板选项点 `Button.onClick`；官方对话用 `DialogueUI.Confirm` 与选项点击；清场以玩家为伤害来源走 `Health.Hurt`；夜里用 `DevForceNight`；帧时间与 `SkyIslandFrameProfile` 分项计时同 `SKY_PERF_*`。断言求值在 `…Asserts.cs`，名单与纯判据在 `…Judges.cs`。
