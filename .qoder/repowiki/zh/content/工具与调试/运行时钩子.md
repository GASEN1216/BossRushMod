...
2026-08-31 的实机日志确认，当前 Harmony 的 class processor 会把普通类型里名为 `Cleanup` 的业务
方法误认成补丁 cleanup 回调。启动扫描现先检查类级或方法级 `[HarmonyPatch]` 元数据，再创建
processor；仍保留逐补丁类隔离，单个真实绑定失配不会拖掉其余补丁。共享兼容层另对动态角色的
MagicBlend 首状态时序做有界延迟：仅在 `MagicBlending.Start` 尚未建立 Playable 时拦截，初始化后
且 Animator 仍处于原状态才重放，正常角色无额外路径。

2026-09-01 的第二次 F3 报告确认，共享 `ModeEFSpawnPostprocess` 队列也被标准模式的随机事件
Boss 使用。模式组更新现在先推进该 scheduler，再允许 WavesArena early-return；否则标准模式任务
会永久停在 Pending，直到下一模式把队列清成 `scheduler_cleared`。空队列每帧只做一次 Count 判断。
