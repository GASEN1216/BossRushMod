...
`AffinityManager.Load` 先建立当前槽写屏障。只有确定缺少 key，或 `AffinityJsonSerializer` 完整解码所有 NPC 记录后，才允许业务变更、存档收集与退出写入。已存在但为空、格式损坏、字段类型错误或读取抛异常的记录保持只读；重新成功加载后才能恢复写入。解码使用共享 `BossRushJsonParser`，先构建候选集合，拒绝重复 NPC ID 和部分成功。旧记录缺少 `lastGiftDay`、`lastChatDay`、`lastDecayCheckDay` 时均保留 `-1`，首次衰减检查只初始化日期，避免追扣到第 0 天。

赠礼入口调用 `NPCGiftSystem.CanOpenGiftSelection`；已婚玩家在当日已送礼后仍可打开选物容器，具体物品继续由 `GiveGift` 判定。普通重复礼物仍拒绝，同配偶戒指返还，给其他 NPC 的戒指继续触发既有花心惩罚。叮当和羽织的礼物、解锁、治疗反馈缓存按取用时的 `L10n.IsChinese` 失效。NPC 商店显式持有独立展示 Item，关闭与打开失败时销毁展示物品树，不销毁已进入背包或装备槽的实例。

教堂资源注册保持可恢复旧档；`WeddingBuildingRequirementsPatch` 在官方 `BuildingInfo.RequirementsSatisfied` 每次查询时检查当前槽的历史满好感资格和写屏障，避免从 A 槽泄漏到 B 槽。站位扫描与缓存回读通过 `WeddingBuildingRuntimePolicy` 排除地下资源模板及非当前场景对象。教堂在一起天数和回忆入口的文本缓存也包含语言。

验证：`NpcAuditFixes` 直接执行好感读写、旧档默认值、文本缓存与教堂资格/场景判据；守卫和离线执行不证明 Unity 建造菜单、配偶站位、商店 Item 数量或实际中文字体的显示，这些仍须实机确认。
...
- 流程
  ...
  - 普通礼物：计算增益、反应类型、增加点数、更新最后赠送日与反应、显示气泡与动画；数值反馈只有 NPC 身侧「+N」浮字（2026-09-23 起不再弹进度横幅，见文末）。
  - 成功后可能触发结婚流程。
- 算法
...
`Integration/Affinity/AffinityUIManager.cs` 的关系提示采用共享深色皮肤、320×104 留白和 20/16 号文字，原有心形图标保留；名称、等级、进度条分区，装饰与文字均透传点击。进度条补了纯色 sprite：此前它是 `Image.Type.Filled` 却没有 sprite，Unity 会退回整块矩形、`fillAmount` 被完全忽略，好感度条一直显示满格。`Integration/UI/ImageViewerUI.cs` 的图片通过锚点和 preserveAspect 自动适配画布，上下各留 110px，避免 4K 二次放大；标题/提示绑定游戏字体，全屏遮罩用 `BossRushUIColors.BackdropStrong`。资源失败时明确提示“图片暂不可用”，不再创建大渐变占位图或无限显示“加载中”。`Integration/Codex/CodexView.cs` 的高度使用共享逻辑视口。原输入与资源归属流程保持不变；高清屏、长标题、加载失败与重开待实机复测。

## 2026-09-23 审美整改（COMPAT，未实机）

- 好感反馈：聊天没加分（当天已聊 / 满点）时不弹任何东西；加分与送礼只在 NPC 身侧冒「+N」浮字（自建 HudOverlay 画布、跟随 NPC、心形 + 28 号数值 + 14 号「Lv.x · a/b」进度行，弹出 / 上浮 / 淡出，unscaled + 暂停门，NPC 不在画面内不弹）；送礼不再同时弹大横幅；升级的「提升到 Lv.x / 解锁 / 折扣」合并成一条多行通知，并播官方 `UI/level_up`、NPC 冒心。花心惩罚横幅保留。
- 头顶心形 / 心碎气泡（`NPCBubbleAnimator`）：0.18 秒弹出、缓慢上浮 0.25 m、最后 0.3 秒淡出再销毁，计时改 unscaled。
- 看图器：图片装进圆角卡（描边 + 外投影，按图片宽高比贴合），打开淡入、关闭 0.12 秒淡出；换图不重播。
- 净化点商店价格前缀双语化，买不起用 DangerText。
