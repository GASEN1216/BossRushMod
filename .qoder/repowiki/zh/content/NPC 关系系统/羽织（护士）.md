...
赠礼与婚后对白继续由 `NurseAffinityConfig` 在取用时解析中英文。羽织以值班、诊疗记录等具体生活细节表达关心，保留嘴硬但在意玩家的口气；收过礼物时直接说今天已收、明天再带，不让角色念“额度”。本次只改三个双语句对，不改事件键、台词池条数、好感数值或每日赠礼规则。
...
- 回忆当天：可在婚礼教堂重播结婚过场（不改变状态）。2026-09-23 起回忆回放可按 Esc 跳过（1 秒后右下角提示，跳过时单独播放的音频一并淡出停止）；第一次结婚过场仍不可跳过。过场进出都有淡入淡出（先 0.5 秒淡到黑、Prepare 完同帧开播画面与音频，结束 0.6 秒淡回）。
