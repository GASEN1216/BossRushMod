...
寄存读取只接受三张列表长度、元素与提交代号均有效的主快照或备份。两者都不能完整恢复时 `DepositDataManager.CanWrite` 为 false，禁止存入、取回和保存，不把空缓存写回；恢复备份只交换内存，不立即改写故障字节。原全局 key 与提交格式保持兼容。

自有寄存商店通过 `StockShop.Buy` / `Sell` 前缀在官方交易发生前接管；单件、批量存取共用 `DepositTransaction`，持有服务代次、存档槽、玩家与收费 NPC。关闭/重开会使旧请求失效，旧 finally 不能释放后继 owner。取回先恢复完整 Item 树，等待后验证 owner，再收费、交付并按记录对象身份删除；恢复或交付失败保留原记录，尚未交付的费用按原币种退回，清理临时实例。展示用简化实例不能成为实际交付物。

“开启下次扫箱”先通过 `ReleasePendingSweepResultToPlayer` 返还旧箱全部剩余物品，再销毁空箱并启动下一次付费确认；任何剩余物品返还失败都会保留旧箱 owner 并阻止新一轮。

2026-09-23（owner 实测第 13 条）：旧箱剩余物品改为整箱静默寄进快递站——`CourierService.BufferItemsSilently` 按官方 `PlayerStorage.Push` 的顺序先序列化入 `PlayerStorageBuffer.Buffer`、再 Detach + DestroyTree，全部写完只 `SaveBuffer` 一次，只推一条「阿稳已把扫箱箱子里的 N 件物品放进快递」汇总横幅；不再逐件 `SendToPlayer`（背包满时每件一条官方横幅，会挡住别的消息）。只寄扫箱产出的物品：`SetPendingSweepResult` 在排序合并后用 `CaptureSweepProducedItems` 记下箱内物品，玩家自己塞进箱子的东西仍走官方逐件交还背包，否则「开启下次扫箱」就成了免快递费的寄件口（同类堆叠被玩家合并进扫箱物品的那一部分会随之寄出，属已知小口子）。`PlayerStorageBuffer.Instance` 未就绪或单件序列化失败时才退回逐件 `SendToPlayer`，交付不了的留在箱里。NPC 销毁、Mode E/F 收尾、运行时重置等其它释放路径共用同一实现，所以旧箱在任何收尾路径都进快递。快递服务关闭时的自动发送 `ExecuteAutoDelivery` 也改走这个帮助方法，缓冲未就绪时不收费。

首次见面对话被场景或 actor 销毁中断时停止后续赠书，已见面标记延后至完整对话结束，使中断的首遇可以重试。共享对话取消、寄存故障与代收箱交付顺序已有隔离回归；真实游戏里的完整配件树、背包满载返还和收费 UI 仍须实机确认。
...
- 动态计费：根据寄存时长与物品价值计算费用，支持“全部取出/全部丢弃”链接按钮。
  - 2026-09-23：链接颜色走 token（取出 SuccessText、丢弃 DangerText、钱不够时价钱标 DangerText），悬停提亮并播官方 UI/hover、UI/click；
    “全部丢弃”不再一点就删，先弹确认框（`OriginalConfirmDialogueAdapter.ExecuteOverActiveView`：不关商店、默认选中“取消”、
    吃掉官方取消键），确认后重新校验会话代号与写入状态才丢弃（守卫 `StorageDepositDiscardConfirmGuard`）。
  - 快递 / 寄存 / 扫箱克隆的官方整理按钮不再改 ColorBlock，保持官方外观；扫箱“开启下次扫箱”在背包满时把玩家自己塞进代收箱的东西留在箱里、汇总提示一条，不再免费寄走。
...
## 2026-09-17 玩家对白与新内容引导（COMPAT）

`Localization/LocalizationInjector.cs` 的 `COURIER_DIALOGUES` 保留旧行次序并在末尾追加新内容。随机气泡补到宿命回响、黑市鸭王杯、六章征程、Jeff 航向仪与天空岛入口、遗种巢、词缀锻造、后山和新装备获取。它是随机闲聊，不是按玩家进度推进的任务播报；关键解锁仍由各系统负责。

台词改用一两句具体提醒，去掉重铸保证高属性、集齐成就必有神秘大奖、各模式死亡都无损等无代码支撑的承诺。首遇仍是四句，保留赠书与寄存介绍。旧本地化 key 不改名、不重新排号。

`CourierNPCController.ShowRandomDialogue()` 按当前中英文长度给随机气泡 3–5 秒阅读时间，调用官方气泡 duration 参数，不占用玩家输入。时长与排版仍需实机确认。
