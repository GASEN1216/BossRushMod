...
晴禾 / 苇白在岛上满足戒指条件即可结婚，婚礼收尾移除当前实例，后续驻留 / 跟随仍走既有永久配偶异步 owner。`AttachPermanentParts` 新接 `SkyIslandResidentInteractable.AttachPermanent`，普通生成与婚后恢复共用幂等子选项，不复用旧岛上闭包：岛上解析当前会话并使用原服务，基地只读当前槽的进度与下一步。配偶在岛上恢复时还可挂同 ID 的航路给予者；装置先接管也继续保留，不造第二套故事。
