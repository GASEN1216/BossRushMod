...
- **启动期绑定自检**：`Common/Infrastructure/HarmonyBindingSelfCheck.cs` 在补丁应用后只读校验全部 `[HarmonyPatch]` 类的目标方法是否实际挂载本 Mod 补丁，输出 `verified/total` 与失败清单；不 apply、不补装，`TargetMethod(s)` 动态选目标的补丁类跳过不计入分母。
