...
- [HarmonyBindingSelfCheck.cs](file://Common/Infrastructure/HarmonyBindingSelfCheck.cs)
- [DebugAndTools.cs](file://DebugAndTools/DebugAndTools.cs)
</cite>
...
### Harmony 逐类应用与启动期绑定自检（2026-08-27 兼容性加固）
Harmony 补丁的应用与校验方式已从"整程序集一把梭"收口为可观测的逐类流程：

- **逐类 apply**：`AlwaysOnRuntimeHooks` 不再调用 `harmony.PatchAll()`，改为 `ApplyHarmonyPatchesPerClass(harmony)`——同程序集、同类型枚举顺序、同 `CreateClassProcessor().Patch()`，全部成功时与 `PatchAll` 逐字等价；差别仅在失败路径：单个补丁类目标失配时只有该类失效，其余补丁照常应用，失败清单经 `CriticalLog` 汇总输出（官方更新改坏单个目标方法不再导致全部补丁静默失效）。
- **启动期绑定自检**：`Common/Infrastructure/HarmonyBindingSelfCheck.cs` 在补丁应用后只读校验全部静态目标 `[HarmonyPatch]` 类：逐个声明的 Prefix/Postfix/Transpiler/Finalizer 必须在对应 PatchInfo 集合同时匹配 owner 和 MethodInfo，不能由同目标其他类代替；类没有可核验方法也不算通过。输出 `verified/total` 与失败清单；不 apply、不补装，`TargetMethod(s)` 动态选目标的补丁类跳过不计入分母。2026-09-05（COMPAT，CR-2026-09-05-020）补齐此精度，真实 Harmony 部分挂载执行负例见 `tests/fixtures/HarmonyBindingSecondReview/`，仍需游戏启动 smoke。
- **CriticalLog**：`DebugAndTools` 提供的非 `[Conditional]` 契约级失败日志（按 key 去重、上限 64 条），保证正式构建（不含 `BOSSRUSH_DEV` define、`DevLog` 被整体编译删除）下 Harmony/反射/数据表加载失败仍在玩家日志可见；成功路径零新增输出。
- **急切反射缓存改名**：`Common/Infrastructure/ReflectionCache.cs` 更名为 `BossRushEagerReflectionCache`，消除与懒缓存 `Common.Utils.ReflectionCache` 的同名歧义，成员与行为逐字不变。

结构由 `tests/ArchitectureStructureGuard.py` 钉住：禁止回退 `harmony.PatchAll(`、要求逐类 helper 与自检接入、OnDestroy 路径重置去重状态。

章节来源
- [AlwaysOnRuntimeHooks.cs](file://Utilities/AlwaysOnRuntimeHooks.cs)
- [HarmonyBindingSelfCheck.cs](file://Common/Infrastructure/HarmonyBindingSelfCheck.cs)
- [BossRushEagerReflectionCache.cs](file://Common/Infrastructure/BossRushEagerReflectionCache.cs)
- [DebugAndTools.cs](file://DebugAndTools/DebugAndTools.cs)
