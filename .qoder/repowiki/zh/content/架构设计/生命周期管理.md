...
- 症状：无法定位场景加载时序问题。
  - 排查：用 `compile_dev.bat` 出 dev 构建（注入 `BOSSRUSH_DEV`）后观察 `OnSceneLoaded` 与各模块的 DevLog；契约级失败另有 `CriticalLog` 在正式构建下也会输出。
...
## 结论
BossRushMod 通过 ModBehaviour 统一编排、BossRushRuntimeModuleHost 集中调度、SceneRuntimeContext 安全传递场景信息、BossRushEventBus 解耦通信，形成了健壮且可扩展的生命周期管理体系。ArchitectureSentinelRuntimeModule 为架构级监控提供了扩展点。配合 dev 构建的 DevLog 与正式构建也会输出的 CriticalLog，可有效定位与解决生命周期相关问题。建议在新增模块时遵循统一接口、做好异常隔离与资源清理，确保长期稳定性。
...
- 观测引擎场景事件：
  - 模块侧统一走 `IBossRushRuntimeModule.OnSceneLoaded(SceneRuntimeContext)`，不要各自反射订阅引擎事件。
  - 需要更细的加载时序时出 dev 构建看 DevLog；契约级失败由 `CriticalLog` 在正式构建下也上报。
  - 根据日志调整模块初始化时机与清理逻辑。

> 注：早期曾有一个 `TeleportDebugMonitor`（反射订阅 `SceneLoader.onStartedLoadingScene` /
> `onFinishedLoadingScene` / `ActiveSceneChanged` 的独立 MonoBehaviour）用于逆向学习官方传送流程。
> 它从未被任何代码挂载到 GameObject 上，属已完成使命的死代码，已于 2026-08-27 删除。
