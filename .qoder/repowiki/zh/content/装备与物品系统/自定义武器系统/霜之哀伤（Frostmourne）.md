...
- 2026-09-23（VA-11 / VA-12）：挥砍改为照新武器的逐点手撒（每米 26 颗、单帧最多 12 个采样点，自动发射关闭），尺寸显式 0.16–0.28 m（不再用 `startSizeMultiplier` 把常量模式放大到 1.8 m），挥击结束后留 0.35 秒让粒子自然淡尽，光 range 1.8 并随拖尾衰减到 0；手持冰焰改 World 空间、寿命封顶 0.8 秒，材质重染改为「换色相、留亮度」（`RetintIceKeepIntensity`，读写原始值，覆盖 `_BaseColor` / `_TintColor` / `_Color`）。
