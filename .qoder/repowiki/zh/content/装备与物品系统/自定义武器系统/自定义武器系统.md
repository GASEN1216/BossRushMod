...
三把近战的拖尾参照焚皇断界戟，采用世界空间、白色初始色加一次颜色渐变；粒子缩为细尾迹，挥击结束后保留 0.3 秒自然消散。**发射方式是逐点手撒**（2026-09-19 实测修正）：`rateOverDistance` 会把一帧该撒的量全撒在当帧的那一个位置上，而挥击用缓出曲线、起手一帧就吃掉小半条弧，结果是起手堆成一坨、后半条几乎是空的；现在自动发射全部关闭，`EmitAlongArc` 在上一帧与这一帧的角度之间按弧长插值补点，逐点 `ParticleSystem.Emit(EmitParams)`。五件装备的公共爆发环改用 128 像素空心环，碎片继续使用圆形精灵。挥击入口只接受主玩家。
...
| 组件 | 职责 |
| --- | --- |
| `NewWeaponSwingFx` | 三把近战的挥砍拖尾。粒子系统运行时构造，材质取 `RingParticleEffect.GetSharedParticleMaterial()`（本轮从 `CreateMaterial` 提取为 internal 静态入口，全 Mod 共享一份 Alpha Blended 材质 + 64×64 程序化渐变贴图）。`emission` 全程关闭，粒子由 `EmitAlongArc` 沿挥击弧逐点手撒（每米约 26 颗、单帧最多 12 个采样点、上限 192 颗），`sizeOverLifetime` 让尾部收细。池上限 8。 |
