...
## 2026-09-20 读档与召唤补漏（COMPAT）

三件新近战通过 `NewWeaponRuntime.RegisterRuntimeConfigs` 接入既有自定义物品恢复服务；工厂的 prefab 注册不能替代这一层。实例补配保留现有耐久/维修损耗，并在资格、估价、选物及手持兼容入口恢复 RF 差值。召唤法杖正常收势不再取消慢加载请求，离手事件则立即撤销旧代数。生产方法执行回归见 `tests/fixtures/ManualEquipmentRecovery/README.md`，实际加载耗时与握姿仍由 owner 实机确认。


## 2026-09-19 模型与反馈修复（COMPAT）

`NewWeaponConfiguratorCore.ApplyModelFit` 在模型注册、绑定手持代理之前修正视觉层，标记子节点保证重复配置不叠加。工作区现有 bundle 的法杖/长矛 Mesh 原点约居中，但视觉子节点统一上移 0.8 米；现移回手部握点中心，保留作者旋转与比例。毒蛇匕首以手部挂点为中心缩为 80%，其视觉偏移由 -0.6 米同步收近为 -0.48 米。模型数据核对为 L1，握姿观感仍待 L3。

三把近战的拖尾参照焚皇断界戟，采用世界空间、白色初始色加一次颜色渐变；粒子缩为细尾迹，挥击结束后保留 0.3 秒自然消散。**发射方式是逐点手撒**（2026-09-19 实测修正）：`rateOverDistance` 会把一帧该撒的量全撒在当帧的那一个位置上，而挥击用缓出曲线、起手一帧就吃掉小半条弧，结果是起手堆成一坨、后半条几乎是空的；现在自动发射全部关闭，`EmitAlongArc` 在上一帧与这一帧的角度之间按弧长插值补点，逐点 `ParticleSystem.Emit(EmitParams)`。五件装备的公共爆发环改用 128 像素空心环；2026-09-23 起环用加色 HDR 材质、EaseOut 扩散、(1-t)^1.5 淡出，中心加一闪即收的亮芯，碎片改为沿速度拉伸、先快后慢的一次性粒子（`BossRushFxKit.PlayBurst`），灯在前半程衰减完。挥砍主体改成一条 TrailRenderer 刀光（条带贴图、头宽尾尖、0.16 秒淡尽），粒子降为每米 10 颗的 HDR 星屑；配色按原版画风降了饱和。挥击入口只接受主玩家。

雷电戒指满层提示移除字体不支持的闪电 emoji，释放先画胸口到目标的电弧、目标头顶落雷与身体光环，再执行附加伤害，避免目标在 Hurt 中销毁导致表现中断。能量盾回补逻辑仍是实际伤害的 30%、最多 25 HP、0.5 秒冷却，致死攻击不能被回补救回；头顶通过官方 `FX.PopText` 显示满血钳制后的实际回血值。

验证：`EquipmentManualFindingsGuard` 通过，耐久/缩放/尾迹清空/NextFrame/空心环五项副本变异均实跑转红并按 SHA-256 还原。`SetBonusCoroutines` 逐字抽取能量盾生产回调，验证正背面、瞄准转向、比例、上限、冷却、实际飘字与致死/卸装早返（L2）。渲染结果与帧耗仍需实机确认。
...
| 组件 | 职责 |
| --- | --- |
| `NewWeaponSwingFx` | 三把近战的挥砍拖尾。主体是 TrailRenderer 刀光（`BossRushFxKit` 条带贴图 + 加色亮度档，池对象复用前 `Clear()`）；粒子是点缀星屑（GlowDot、HDR 高档）。`emission` 全程关闭，粒子由 `EmitAlongArc` 沿挥击弧逐点手撒（2026-09-23 起每米约 10 颗、单帧最多 12 个采样点、上限 192 颗），`sizeOverLifetime` 让尾部收细。池上限 8。 |
