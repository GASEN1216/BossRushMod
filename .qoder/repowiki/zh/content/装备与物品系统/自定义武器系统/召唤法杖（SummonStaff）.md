...
## 2026-09-20 慢加载与离手取消（COMPAT）

`TotalActionDuration` 的 1.2 秒只结束施法动作，不再撤销尚未完成的角色创建。异步请求仍按代数、当前主角、生命、手持和场景检查后才接纳实体；新施法、非正常中断、销毁或停用会撤销旧请求。

手持变化由管理器立即调用 `CancelPendingSummons`，因此离手后在异步完成前迅速拿回，也不能复活旧请求。NPC 的手持事件不影响主玩家。迟到实体仍在原 `SpawnAlliesAsync` 拒绝分支销毁，数量/生命/伤害/冷却不变。`ManualEquipmentRecovery` 覆盖慢请求、动作收势、换走再拿回、死亡、切图、停用与销毁；尚无真实帧耗采样。
