...
2026-09-22 战斗审计修复（COMPAT）：龙套的 `OnHurt` 发生在官方扣血之后，只保留按火元素比例回补 80% 的现有规则，不再清零调用方复用的 `DamageInfo.elementFactors`，避免一次爆炸的后续目标丢失火伤。`PlayerLavaZone` 捕获施法者和场景，按施法者当前阵营判敌并显式排除本人，离开原场景或 owner 死亡后销毁。熔浆与逆鳞棱彩弹均标记效果伤害；棱彩弹搜索和命中共用实际 owner 的敌对判据，兼容 Mode E 营旗。旧快照中的“在 OnHurt 里清零即免疫”解释已过时；共享元素列表不变和回补金额有执行回归，真实命中仍需 L3。

## 2026-09-22：普攻附带的请求归属与结算冷却（COMPAT）

`FrostBiteStep` / `ThunderBiteStep` 在延时结束后先核对套装激活态与 `setBonusGeneration`，只有当前代数才能释放自己的 pending 标记。旧激活请求醒来后直接退出，不能放开新激活请求的排队闸；结算入口再检查 1.1 / 1.4 秒冷却，重复交付也不会造成额外伤害或雷噬扫描。死亡、卸装、过图仍由现有代数推进作废旧请求，扫空和目标提前死亡仍不消耗冷却。

`SetBonusCoroutines` 直接执行两套生产协程，覆盖旧请求/新请求/第三次命中的交错及同代重复交付，并验证冷却结束后下一发可用。`SetBonusLifecycleGuard` 检查“代数 → 释放 pending → 复核冷却 → 表现/扫描”的顺序；守卫和回归均做隔离反向验证。数值、效果和装备获取方式不变，实机穿脱与战斗手感仍需 owner 验证。
...
- [SetBonusFx.cs](file://Integration/Bonus/SetBonusFx.cs)
- [BossRushFxKit.cs](file://Common/Effects/BossRushFxKit.cs)
...
- 冰霜套装（霜冠+寒冰铠甲）「寒冰之护」：冰抗提升、冰伤 50% 转治疗、每件寒冷防护 +1；**普攻附带**「霜噬」（命中即追加 5 冰伤 + 35% 冻结被打中的目标 / 1.1 秒内置冷却）；受击概率冻结近身攻击者（带冷却）；淡蓝双眼呼吸光 + 脚下霜雾。
- 雷霆套装（雷神之角+雷霆战甲）「雷霆之怒」：电抗提升、电伤 50% 转治疗、每件风暴防护 +1（风暴天气免疫）；**普攻附带**「雷噬」（命中点 4 米内最多 2 个**其它**敌人各 7 电伤 / 1.4 秒内置冷却 / 不续跳）；受击概率以玩家为中心释放电击 AOE（4 米 / 30 电伤 / 3 秒冷却 / canHurtSelf=false）；青白双眼电闪 + 肩部环境电弧。
...
- **表现层**（`SetBonusVisuals.cs` 只留门面，实现在 `SetBonusFx.cs` 与共享特效层）：眼光复用 `DragonSetBonus.FindHeadTransform` / `cachedHeadTransform`，表现是 `SetBonusEyeGlow`（两颗面朝镜头的 6 cm HDR 亮点 + 一盏 0.6 m 弱补光，闪烁按时间 Perlin，不按帧掷骰）；爆发环 `SpawnSetBurst` 转调 `NewWeaponFx.PlayBurst`（贴地空心 HDR 环 + 亮芯 + 粒子碎片），默认不开灯，只有冻结反击传 `withLight`；电弧为 `SetBonusArcPool`（每道外晕 + 白热芯两根 LineRenderer，条带贴图、两头收尖、闪烁淡出，上限 12 道，停用时 `DestroySetArcPool`）；霜雾为 `RingParticleEffect` 子类 `FrostMistEffect`（2026-09-23 起是 World 空间贴地薄霜 + 冰晶星点，整件不超过 24 颗活粒子）。音效 `Assets/Sounds/SetBonus/*.wav` 由 `tools/gen_setbonus_sfx.py` 程序化合成，`compile_official.bat` 部署段照 BGM 块追加拷贝。
...
- 状态变化：无持久状态，仅伤害瞬间处理。
- 视觉效果：龙眼暗红眼光（2026-09-23 起同 `SetBonusEyeGlow`，自带呼吸，不再用协程驱动两盏 25 cm 点光）；冲刺残影为 `DragonDashAfterimageFx`（面朝镜头、角色大小的暖橙柔光 + 上飘烟缕，只有第一个残影带弱灯）；龙王冲刺熔浆到点停伤害，视觉停发射、灯淡出后再销毁。
...
## 2026-09-23 特效审美修（VA-01 / VA-02 / VA-03 / VA-05 / VA-06 / VB-27，COMPAT）

- `RingParticleEffect` 不再在 `LateUpdate` 每帧手撒（密度随帧率走），发射只走 `rateOverTime`；出生不再一次撒 10 颗。共享材质 `GetSharedParticleMaterial()` 的 `_TintColor` 改为中性 0.5（`SetVector` 原始值），Legacy 粒子着色器的颜色与 alpha 不再翻倍；贴图换成全 Mod 共用软圆 `BossRushFxMaterials.GetSoftCircleTexture()`。基类默认值按旧的实际生效密度重折（`FlightCloudEffect` 走默认值）。子类可换渲染方式、材质、径向速度、寿命末端色，并在 `OnEmittersCreated` 加点缀层；`StopEffect` 等最后一颗粒子走完再销毁。
- 霜雾改为 3 个 World 空间发射器的贴地薄霜（烟缕贴图、水平公告板、有效 alpha 0.15、由浅冰白沉成冰蓝）+ 冰晶星点层。
- 霜噬 / 雷噬的爆发不再每次开灯；爆发环、电弧、眼光见上文「表现层」。
- 观感只能 L3，清单见 scratchpad 交付报告的「owner 看图清单」（天空岛灶火 / 飞行云雾也用这份共享材质，亮度会随之减半）。
...
`tests/fixtures/SetBonusCoroutines/` 链接完整生产技能协程并逐字提取死亡回调；同时读取能量盾、雷电戒指、冰霜套、雷霆套的完整 `OnHurt`，守护致死 `OnDead → OnHurt` 顺序。当前 12 条执行断言与 4 个生产方法顺序探针通过。Unity 表现与战斗仍需实机。

## 2026-09-20 改版：击杀触发 -> 普攻附带

owner 定：「不是杀死之后才弄效果，而是改为普通攻击就附带效果，然后伤害降低，并内设冷却，
防止 DPS 过高的武器增伤太明显。」

- 触发点从 `Health.OnDead` 搬到 `Health.OnHurt`：两套装的 `OnXxxSetHurt` 先按
  `IsMainCharacterHealth` 分流——敌人被打中走 `TryScheduleFrostBite` /
  `TryScheduleThunderBite`，主角受击仍走原来的转治疗与反制。
- 反 DPS 缩放的三道闸（`SetBonusLifecycleGuard` 逐条断言）：
  1. 内置冷却（霜噬 1.1 秒 / 雷噬 1.4 秒），与射速完全脱钩；
  2. 单次伤害是常数（霜噬 5 / 雷噬 7），**不乘**触发那一击的伤害；
  3. 只认 `!isFromBuffOrEffect` 的直接命中，套装自身伤害不自续。
- 击杀不再触发任何套装技能：`OnFrostSetAnyDead` / `OnThunderSetAnyDead` 只剩主角分支
  （死亡推进 `setBonusGeneration`，作废在飞的结算）。
- 雷霆反震的爆炸特效从 `ExplosionFxTypes.normal`（火焰系）改成 `flash`（闪光），
  实际电光观感仍待实机确认。
- 执行回归 `tests/fixtures/SetBonusCoroutines` 已改成按普攻命中驱动，35 项断言通过。

第二轮补漏（COMPAT）：`SetBonusVisuals` 的命中与范围扫描均按玩家当前阵营调用
`Team.IsEnemy`，不假设玩家永远是 `Teams.player`。Mode E 换阵营后友军不会触发普攻附带，
敌人仍能正常入选；遗种巢随从与 Mode H 观战过滤继续保留。`ManualSeptemberReview` 执行
同队、中立、敌队与随从四种判据；`SetBonusLifecycleGuard` 同步接线并做反向验证。L3 战斗待确认。

第三轮补漏（COMPAT）：**「效果没落地就不扣冷却、不播音效」**。

- `TryApplyFrostFreeze` 过去在 `AddBuff` 之后无条件 `return true`。官方
  `CharacterMainControl.AddBuff` 返回 void，且有三条静默 no-op 路径
  （`buffResist` 命中该 `ExclusiveTag`、同 tag 已有更高 `ExclusiveTagPriority`、
  同 tag 同优先级但现存剩余时间更长）——抗冻的目标正好走第一条。
  现在回读 `target.HasBuff(freezeBuff.ID)` 才算成功；极端兜底的减速也按
  「CharacterItem / 速度 Stat / 协程是否真的拿到」回报真实结果。
  于是打免疫目标不再「播音效 + 出特效 + 扣掉 6 秒反击冷却」却什么也没冻住。
- 霜噬 / 雷噬的冷却从**排队时**挪到**结算时**：
  `frostBitePending` / `thunderBitePending` 挡住延迟窗口内的重复排队，
  真正走到「会造成伤害」那一步才写 `lastFrostBiteTime` / `lastThunderBiteTime`。
  目标在 40~50 毫秒延迟里被打死、中途脱装备切图、或雷噬扫不到其它敌人时，
  冷却不再被白白吃掉——单挑 Boss 时「雷噬扫空」正是最常见的一种。
  反 DPS 的三道闸不受影响：同一时刻最多一条在飞，伤害仍是常数。
- `SetBonusLifecycleGuard` 改成按**位置关系**断言（冷却写入必须在 `StartCoroutine`
  之后、在 `count <= 0` 早返之后、在 `TryApplyFrostFreeze` 成功分支之内），
  而不是查子串是否存在；`SetBonusCoroutines` 新增 8 条执行断言覆盖扫空、
  延迟内死亡与冻结失败三种路径。四个反向探针均在预期位置转红并按字节还原。
  L3 战斗手感仍待实机。
