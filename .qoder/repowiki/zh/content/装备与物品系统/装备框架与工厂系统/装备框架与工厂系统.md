...
2026-09-22 审计修复（COMPAT）：`EquipmentAbilityManager.CleanupStatic` 经既有只读实例查找取得实际管理器，再注销能力并销毁常驻对象；覆盖使用独立静态字段和自有 `Awake` 的旧子类，最终由 `OnDestroy` 释放持握事件和输入缓存。飞行动作零体力时仍执行下降与松手逻辑，目标垂直速度跨物理步保留，停止或重置才清零；飞行入口统一检查官方输入、暂停、光标和活动界面。`EquipmentResourceScene` 执行生产管理器，覆盖 29 项场景、输入和最终销毁断言。

通用装备登记只安装焚天龙铳运行时监听。龙铳弹幕提取与火焰池预热由主玩家实际持握触发，离手、死亡、过图和清理后在等待点复核 owner / generation；清池同时复位 running 和目标容量。荒野号角创建坐骑也持有场景与请求代次，等待中的重复使用被拒绝，迟到结果销毁而不绑定旧主角。对应当前源码优先于下方历史流程图；验证止于 L1 / L2，输入和物理手感仍待 owner 实机确认。
