...
> 2026-09-20 资源校准更新：owner 已确认第一版雷霆佩戴方向，并指出头顶穿模；本轮提高间隙并完成另外五顶校准，两龙参考保持原样。官方挂载清零 EquipmentModel 根节点位置/旋转、不清缩放；根节点单位变换，校准全部在网格子节点。雷霆和霜冠虽都 Z-up，但正面轴相反；长角、帽绳与飘带不能决定盔壳中心。唯一数值源 `tools/helmet_fit_profiles.json`，同步脚本 `tools/helmet_fit.py`，作者工程由 `HelmetFitUtility` / `HelmetFitBundleBuilder` 应用并回读，天空岛生成与全量构建已接校验。第一版雷霆方向属 L3 owner 目检，本轮其余结果属 L1/L2，实机仍待试戴。操作见 `docs/guides/头盔佩戴与装备尺寸校准.md`，本轮记录见 `docs/guides/头盔佩戴与装备尺寸校准.md`。

> 2026-09-25 作者源一致性修复（OPERATIONAL）：本机作者副本仍保留旧霜冠姿态，已按仓库校准表同步并只重建 `frost_set`，同包寒冰铠甲也对齐已发布值。Unity 校验通过，新包与仓库现有包逐字节一致；作者 `HelmetFitExport` / `ResourceRelease` 已对齐。未重新部署游戏，佩戴观感无本轮 L3。
