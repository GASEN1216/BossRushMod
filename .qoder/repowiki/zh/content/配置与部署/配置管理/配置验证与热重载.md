...
LootBlacklistRegistry 通过 BossRushJsonParser 只读取 itemIds 数组，错误类型拒收并使用原黑名单 fallback。MapSpawnPointRegistry 的每个 Vector3 必须恰好三项有限数值，非法向量不能默认为世界原点；合法 [0,0,0] 仍有效。BossRushJsonValue 的 double 转 float 溢出返回失败/fallback，避免把 Infinity 传到坐标和移动参数。AuditCoreParsing 执行这些生产解析路径。
