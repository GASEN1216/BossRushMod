...
- [gen_codex_art.py](file://tools/gen_codex_art.py)
- [CodexArtGenerationPropertyTest.py](file://tests/CodexArtGenerationPropertyTest.py)
- [AchievementIcons 回归](file://tests/fixtures/AchievementIcons/README.md)
</cite>
...
- 图标加载：优先读取独立 PNG，兼容旧 AssetBundle；统一缓存并清理自有 Sprite / Texture。
...
### 图标加载与缓存
- `GetSprite` / `GetTexture` 优先使用 `Assets/achievement/{成就ID}.png`，缺失或解码失败时回退 `Assets/achievement/achievement_icons` 旧包；最终仍缺失时由界面保留默认图标。文件名只允许字母、数字、下划线和连字符，调用方传无扩展名 ID。
- Sprite 与 Texture2D 双缓存避免反复读取。PNG 创建的两类对象、由旧包 Texture 创建的 Sprite 都由加载器持有；`ClearCache` 清理这些自有对象，直接借用的包内 Sprite / Texture 不在此处销毁。`ResetStaticCaches` 沿用统一卸载入口，连续清缓存保持幂等。
- 遗种巢五项与图鉴五项成就的独立图标规格集中在 `tools/gen_codex_art.py`，尺寸为 256×256。新增成就图标可直接部署 PNG，无需为补图重打旧成就包。
- 旧的 36 张成就图标原先在 `achievement_icons` 包里被 importer 压到 64px，进游戏一片糊。`tools/sync_achievement_icons.py` 把作者工程 `Assets/AchievementIcons/` 的 1024 源图落成 256 PNG 放进 `Assets/achievement/`，**走 PNG 优先这条路，不重新导入、不重打包就能立刻变清楚**；旧包仍保留为没有部署 PNG 时的回退。作者工程侧的 importer 也已改成 256（`tools/apply_unity_texture_policy.py`，守卫 `tests/UnityTextureImportPolicyGuard.py`），重打后的 `achievement_icons` 从 69 KB 变为 902 KB。
- 生成脚本保留原有图鉴立绘、物品、词缀和事件规格，同时提供遗种巢与征程告示板的 256×256 纯白手绘建筑图标。建筑输出会保留透明度并统一可见像素 RGB 为白色。
- 预览命令 `python tools/gen_codex_art.py --list --filter Assets/achievement/ --filter Assets/buildings/` 不生成文件、不调用外部工具。`--filter` 可重复指定，按路径或 ID 子串取并集；`--force` 只重出选中条目，并从新 raw 开始。先完成暂存生成、抠图与归一化，再替换 raw / cut / 最终资产；生成或抠图失败不会使用陈旧缓存冒充新图。
- `normalize` 使用 alpha 合成居中，避免二次乘 alpha 导致半透明边缘变暗。`tests/CodexArtGenerationPropertyTest.py` 直接执行生产像素与文件流程；外部生成器由替身隔离。`tests/fixtures/AchievementIcons/` 链接完整生产加载器，验证 PNG 优先、包回退和资源生命周期；Unity 原生解码与显示效果仍需实机。
...
领奖由 AchievementRewardJournal 保存全局意向与当前槽收据；现金和收据同批落盘后才提交原全局已领标记。中断后原槽重试不重发，未结清意向阻止其它槽抢领。新增 key 及格式见 docs/contracts.md §3。治疗监听在每次官方关卡初始化时重绑当前主角，销毁时退订所绑定 Health；成就页、条目与弹窗随模块退出清理。SkyIslandStory 隔离回归覆盖领奖失败、重试和跨槽，UI/事件实机仍待 owner。
