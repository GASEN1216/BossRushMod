...
2026-09-22（COMPAT）：普通角色的采集资格为“运行时 Boss 标记，或当前官方 Boss 名单 key”。`Cname_RaiderIce` 的官方 preset 标记为 false，仍须经名单收录并记录四值，否则锁定卡与全收集不可完成。既有归属、友军、基地、Mode H 过滤不变，丧尸 marker 先决定归属；官方非 Boss 生物不因展示目录而获得资格。`ContentThirdReviewFixes` 直接执行生产采集与 Codec，覆盖该真实差异及排除项，实机保存仍由 owner 复测。
...
完整证据、取舍与实机清单见本地交付报告 `docs/reports/reviews/2026-09-18-鸭皇图鉴生产审核.md`（local-only）。
...
## 2026-09-23 审美整改（COMPAT，未实机）

标题栏与进度行不再铺底色（圆角 Header 色块 + 直角进度条压在圆角面板里，交界露缺口），改用两条分隔线，标题 28 号、前面一根 Accent 竖条；右上「×」改幽灵按钮（常态透明，悬停显出 Danger 底）；筛选与详情「关闭」走共享次级按钮。卡片 168×236（原 210）：名字 17、击杀 15、最快 14 号，统计行关自动缩字、框高 26。第三级占位改成真正的圆盘 + 描边环（`GetRoundedSprite(32)` 按 Simple + 保持宽高比铺满正方形）。里程碑刻度走细条档。主面板关闭 0.12 秒淡出后 `SetActive(false)`，详情弹层关闭走 `BossRushUIKit.PlayCloseAndDestroy`。目录、存档与采集逻辑不变。
