...
4. `DayIndex++`
5. `Store` 接受候选后立即消费这一天的计时，再请求协调器物理保存；Store 拒绝则保留计时并退避
6. 出刊提示：在基地立即横幅，否则挂起到回基地再发

2026-09-25 修复 `CR-2026-09-25-001`：SaveFile 失败只重试同一份待写状态，不能保留已消费的跨天计时再次推进 DayIndex。隔离回归 `SaveFailureRecovery` 覆盖 Store 拒绝、物理异常后 IsSaving 保持 true、恢复与领取失败反馈。
...
Store 拒绝时权威内存不变，调用方得到明确失败，UI 不得显示成功。Store 接受与物理落盘是两件事：跨日计时只随前者消费一次，硬写失败留给协调器重试；面向玩家的领取入口仍报告物理失败。跨日悬赏先把待发债务随 rollover 提交，再发现金并以第二次候选提交标记领取。
