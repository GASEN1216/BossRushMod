...
> 先读根目录 `AGENTS.md`。涉及 hooks、刷怪、恢复、场景门控时读 `docs/architecture/` 下对应文档（local-only）。

## 职责边界

`Utilities/` 只放跨模块共享的运行时基础设施：运行时 hooks、刷怪核心、恢复监控、场景门控、缓存、run-scoped cleanup、平台辅助。单模块 helper 留在模块目录（`docs/architecture/Utilities去重约定.md`）；新 hook 归位前读 `docs/architecture/Hooks分层约定.md`。

## 规则

- `EnemySpawnCore`、`SpawnPositionHelper`、`EnemyRecoveryMonitor` 是运行时高风险区，调整前读 `docs/architecture/刷怪与恢复系统设计.md`。恢复系统的经验常数是调参结果，不凭直觉改。
